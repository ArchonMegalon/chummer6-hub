...
async Task VerifyPublicLandingProjectionAsync()
{
    ...
    Assert(statusModel?.LaunchHealthRows?.Any(static row => string.Equals(row.Label, "Adoption health", StringComparison.Ordinal) && row.Value.Contains("Current local edge proof passed", StringComparison.Ordinal)) == true, "status page should surface passed local edge proof directly inside the launch-health breakdown.");
