...
await VerifyHostedBoundedContextCoverageWorkflowAsync();
...
void VerifyIdentityEmailDeliveryProviders()
{
    ...
    try
    {
        ...
        {
            Assert(payload.RootElement.GetProperty("from").GetString() == "Chummer Concierge <concierge@chummer.run>", "Emailit payload should preserve the configured sender label.");
...
async Task VerifyHubCommunitySecurityAndDurabilityAsync()
{
    ...
    var campaignSpine = new CampaignSpineService(store, new WorkspaceLifecyclePolicyService(configuration), new CampaignArtifactRegistryBridge(store), supportStore, publicationDraftWorkflow);
    ...
    var malformedFailureBridge = new FleetBridgeService(new HttpClient(new StubHttpMessageHandler(request =>
    {
        if (request.Method == HttpMethod.Post && request.RequestUri?.AbsolutePath == "/api/internal/participant-lanes")
        {
            return new HttpResponseMessage(HttpStatusCode.ServiceUnavailable)
            {
                Content = new StringContent("fleet upstream unavailable")
            };
        }

        return JsonResponse(new { detail = "unexpected fleet call" }, HttpStatusCode.InternalServerError);
    })), configuration);
    var malformedFailureSessions = new BoostSessionService(store, accounts, groups, malformedFailureBridge, rewards);
    var malformedFailureSession = malformedFailureSessions.Create(new CreateSponsorSessionRequest(
        SubjectId: "subject.parse.failure",
        ProjectId: "codexea",
        SubjectLabel: "Parser Failure"));
    malformedFailureSessions.RecordConsent(malformedFailureSession.SponsorSessionId);
    var malformedFailureUnavailable = false;
    try
    {
        await malformedFailureSessions.StartDeviceAuthAsync(malformedFailureSession.SponsorSessionId, CancellationToken.None);
    }
    catch (ParticipationUnavailableException ex)
    {
        malformedFailureUnavailable = ex.Message.Contains("Participation is unavailable", StringComparison.OrdinalIgnoreCase);
    }

    Assert(malformedFailureUnavailable, "plain-text Fleet failures should degrade into participation unavailability instead of leaking JSON parser exceptions.");

    var malformedSuccessBridge = new FleetBridgeService(new HttpClient(new StubHttpMessageHandler(request =>
    {
        if (request.Method == HttpMethod.Post && request.RequestUri?.AbsolutePath == "/api/internal/participant-lanes")
        {
    ...
    var malformedSuccessSessions = new BoostSessionService(store, accounts, groups, malformedSuccessBridge, rewards);
    var malformedSuccessSession = malformedSuccessSessions.Create(new CreateSponsorSessionRequest(
        SubjectId: "subject.parse.success",
        ProjectId: "codexea",
        SubjectLabel: "Parser Success"));
    malformedSuccessSessions.RecordConsent(malformedSuccessSession.SponsorSessionId);
    var malformedSuccessUnavailable = false;
    try
    {
        await malformedSuccessSessions.StartDeviceAuthAsync(malformedSuccessSession.SponsorSessionId, CancellationToken.None);
    }
    catch (ParticipationUnavailableException ex)
    {
        malformedSuccessUnavailable = ex.Message.Contains("Participation is unavailable", StringComparison.OrdinalIgnoreCase);
    }

    Assert(malformedSuccessUnavailable, "non-object Fleet success payloads should fail closed into participation unavailability.");
        ...
        var teableHttp = new HttpClient(new StubHttpMessageHandler(request =>
        {
            ...
            if (request.Method == HttpMethod.Get && path.StartsWith("/api/table/tbl_users/record?", StringComparison.Ordinal))
            {
                if (path.Contains("filterByTql=", StringComparison.Ordinal))
                {
                    ...
                    return existingRecordIdByUserId.TryGetValue(userId, out var filteredRecordId)
                        ? JsonResponse(new
                        {
                            records = new[]
                            {
                ...
                var records = existingRecordIdByUserId
                    .Select(item => new
                    {
                        id = item.Value,
                        fields = new Dictionary<string, object?>
                    ...
                    .ToArray();
                return JsonResponse(new { records });
            }
...
async Task VerifyPublicLandingProjectionAsync()
{
    ...
    Assert(accountSource.Contains("<p><strong>Campaign readiness:</strong> @selectedWorkspaceFirstPlayableSession.CampaignReadySummary</p>", StringComparison.Ordinal), "account work should carry campaign readiness into the selected-workspace summary copy.");
    ...
    Assert(accountSource.Contains("Finish this item before you continue.", StringComparison.Ordinal), "account work should render the blocking-state copy for restore conflict receipts.");
    ...
    Assert(accountSource.Contains("Search prep sets", StringComparison.Ordinal), "account work should expose a real prep-library search flow on the selected campaign card.");
    ...
    var participateContent = await controller.ParticipatePage(CancellationToken.None) as ContentResult;
    Assert(participateContent is not null, "participate page should render through the first-party board proxy.");
    Assert((participateContent!.Content ?? string.Empty).Contains("What should Chummer do next?", StringComparison.Ordinal), "participate page should keep the first-party prompt.");
    Assert((participateContent.Content ?? string.Empty).Contains("Public requests, clear bugs, useful ideas.", StringComparison.Ordinal), "participate page should keep the public board summary.");
    Assert(!(participateContent.Content ?? string.Empty).Contains("data-chummer-participate-frame", StringComparison.Ordinal), "participate page should not reintroduce the iframe wrapper.");
    var authenticatedParticipateContent = await authenticatedLandingController.ParticipatePage(CancellationToken.None) as ContentResult;
    Assert(authenticatedParticipateContent is not null, "authenticated participate page should use the same first-party board proxy.");
    Assert((authenticatedParticipateContent!.Content ?? string.Empty).Contains("What should Chummer do next?", StringComparison.Ordinal), "authenticated participate page should keep the same first-party prompt.");
    ...
    var searchableWorkspaceDetailPage = await accountController.AccountPage(section: null, caseId: null, cancellationToken: CancellationToken.None, workspaceId: workspaceId, prepQuery: "opposition") as ViewResult;
            ...
            Assert((searchableWorkspaceDetailModel?.Experience.WorkspacePrepLibrarySearchHistory?.Any(item =>
    ...
    Assert(dossierPublicationDetailModel?.SelectedCreatorPublicationDraftDetail?.Description?.Contains("Manifest authority: approved-shared-publication-manifest;", StringComparison.Ordinal) == true, "account publication detail route should keep manifest authority attached to dossier moderation detail.");
    ...
    Assert(rejectDossierPublicationResult is RedirectResult { Url: not null }, "dossier publication rejection should redirect back to the publication detail route.");
    var rejectedDossierPublicationDetailPage = await accountController.AccountPage(section: null, caseId: null, cancellationToken: CancellationToken.None, publicationId: dossierPublicationId) as ViewResult;
    var rejectedDossierPublicationDetailModel = rejectedDossierPublicationDetailPage?.Model as AccountPageViewModel;
    ...
    Assert(string.Equals(rejectedDossierPublicationDetailModel?.SelectedCreatorPublication?.PublicationStatus, "rejected", StringComparison.Ordinal), "rejected dossier publications should project the needs-revision posture on the shared creator-publication card.");
    ...
    Assert(resubmitDossierPublicationResult is RedirectResult { Url: not null }, "dossier publication resubmission should redirect back to the publication detail route.");
    var resubmittedDossierPublicationDetailPage = await accountController.AccountPage(section: null, caseId: null, cancellationToken: CancellationToken.None, publicationId: dossierPublicationId) as ViewResult;
    var resubmittedDossierPublicationDetailModel = resubmittedDossierPublicationDetailPage?.Model as AccountPageViewModel;
    ...
    Assert(publicSignalPacketBundle.Packets.Any(item => string.Equals(item.SurfaceId, "feedback", StringComparison.Ordinal) && string.Equals(item.DestinationRoute, "/participate", StringComparison.Ordinal)), "campaign spine public signal packets should emit feedback packets for the public Participate surface.");
    ...
    Assert(hostedProofContracts.Contracts.Any(item => string.Equals(item.SurfaceId, "account_aware_horizon_conversion", StringComparison.Ordinal) && string.Equals(item.ComparisonRoute, "/account/access", StringComparison.Ordinal)), "campaign spine hosted proof contracts should emit account-aware horizon conversion proof on the Installs route.");
    ...
    Assert(registryTruthBindings.Bindings.Any(item => string.Equals(item.SurfaceId, "account_aware_guidance", StringComparison.Ordinal) && string.Equals(item.Route, "/account/access", StringComparison.Ordinal)), "campaign spine registry truth bindings should keep account-aware guidance on the Installs rail.");
    ...
    Assert(privacyBoundedSupportStatus.Projections.Any(item => string.Equals(item.SurfaceId, "feedback_status", StringComparison.Ordinal) && string.Equals(item.Route, "/participate", StringComparison.Ordinal)), "campaign spine privacy-bounded support status should keep feedback status on the shared Participate surface.");
    ...
    authenticatedLandingController.ControllerContext.HttpContext.Request.QueryString = new QueryString("?view=public");
    var publicArtifactsView = await authenticatedLandingController.ArtifactsPage(CancellationToken.None) as ViewResult;
    var publicArtifactsModel = publicArtifactsView?.Model as ShelfPageViewModel;
...
async Task VerifyAiGatewayWorkflowAsync()
{
    ...
    Assert(invocation.Decision.SelectedModel == "claude-opus-4.1", "complex grounded prompts should route to the configured complex model tier");
...
void VerifySpiderWorkflow()
{
    ...
    Assert(decision.DeepAnalysis.RouteDecision.SelectedModel == "claude-opus-4.1", "deep Spider routing should select the configured complex model");
...
{
    string tempRoot = Path.Combine(Path.GetTempPath(), "hosted-bounded-context-coverage-smoke", Guid.NewGuid().ToString("N"));
    Directory.CreateDirectory(tempRoot);
    ...
    try
    {
        File.WriteAllText(
            ...
            JsonSerializer.Serialize(new PublicReleaseManifestDto(
                ...
                ProofStatus: "passed",
                SupportabilityState: "local_docker_proven",
                FixAvailabilitySummary: "Public and signed-in rails still point at the same preview release."),
            new JsonSerializerOptions(JsonSerializerDefaults.Web)),
        ...
        Assert(
            boundedContextCoverage.Projections.Any(item => string.Equals(item.SurfaceId, "orchestration_boundary", StringComparison.Ordinal) && string.Equals(item.Route, "/downloads/install", StringComparison.Ordinal)),
            "hub bounded-context coverage should keep orchestration boundary proof on the install entry rail.");
        Assert(
            boundedContextCoverage.Projections.Any(item => string.Equals(item.SurfaceId, "bounded_context_closure", StringComparison.Ordinal) && string.Equals(item.Route, "/progress", StringComparison.Ordinal)),
