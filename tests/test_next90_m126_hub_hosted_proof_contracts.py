...
class Next90M126HubHostedProofContractsTests(unittest.TestCase):
    def test_verifier_accepts_repo_local_hosted_proof_contract_lane(self) -> None:
        ...
        self.assertIn("next90 m126 hub hosted proof contracts proof passed", result.stdout)

    def test_verify_script_runs_m126_guard(self) -> None:
        verify_script = (REPO_ROOT / "scripts" / "ai" / "verify.sh").read_text(encoding="utf-8")
        self.assertIn("python3 scripts/materialize_next90_m126_hub_hosted_proof_contracts_proof.py", verify_script)
        self.assertIn("python3 scripts/verify_next90_m126_hub_hosted_proof_contracts.py", verify_script)
        self.assertIn("python3 -m unittest tests/test_next90_m126_hub_hosted_proof_contracts.py", verify_script)

    def test_verifier_fails_when_queue_row_drifts(self) -> None:
        with tempfile.TemporaryDirectory(prefix="next90-m126-queue-") as temp_dir:
        ...
        self.assertIn("frontier_id must be 6966685835", result.stderr)

    def test_verifier_fails_when_smoke_loses_account_aware_assertion(self) -> None:
        with tempfile.TemporaryDirectory(prefix="next90-m126-smoke-") as temp_dir:
            temp_root = Path(temp_dir)
            self.copy_sources(temp_root)
    ...
    def run_verifier(self, temp_root: Path, *, queue_path: Path | None = None, design_queue_path: Path | None = None) -> subprocess.CompletedProcess[str]:
        env = os.environ.copy()
        env["CHUMMER_NEXT90_M126_ROOT"] = str(temp_root)
        if queue_path is not None:
            env["CHUMMER_NEXT90_M126_QUEUE_STAGING"] = str(queue_path)
        if design_queue_path is not None:
            env["CHUMMER_NEXT90_M126_DESIGN_QUEUE_STAGING"] = str(design_queue_path)
        return subprocess.run(["python3", str(temp_root / "scripts/verify_next90_m126_hub_hosted_proof_contracts.py")], cwd=temp_root, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def load_queue_payload(path: Path) -> dict:
        ...
        package_marker = "package_id: next90-m126-hub-define-hosted-proof-contracts-for-open-runs-shadowcaster"
