...
class DesktopNativeTrustReceiptTests(unittest.TestCase):
    ...
    def test_verifier_default_current_floor_matches_latest_canonical_guard(self) -> None:
        ...
        self.assertEqual("4e8eb4c1", verifier._current_local_proof_floor_commit())
        self.assertEqual(
            "Pin M102 encoded proof marker guard",
        ...
        self.assertTrue(
            any("commit 4e8eb4c1 pins the M102 encoded proof marker guard" in value for value in verifier.REQUIRED_CANONICAL_QUEUE_LISTS["proof"])
        )
        self.assertTrue(
            any("commit 4e8eb4c1 pins the M102 encoded proof marker guard" in value for value in verifier.REQUIRED_CANONICAL_REGISTRY_LISTS["evidence"])
        )
