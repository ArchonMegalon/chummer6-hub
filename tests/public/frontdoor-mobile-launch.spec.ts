...
test('mobile homepage open-chummer menu gates Build and Play while the player shell stays reachable', async ({ browser }) => {
  ...
  await expect(buildButton).toHaveAttribute('data-disabled-target', '/build');
  await expect(playButton).toBeVisible();
  await expect(playButton).toBeDisabled();
  await expect(playButton).toHaveAttribute('data-disabled-target', '/mobile/player');
