...
test('black ledger map keeps keyboard and fallback access visible', async ({ page }) => {
  ...
  expect(await page.locator('.ledger-world-shell__districts [data-map-district-card]').count()).toBeGreaterThan(3);

  writeMapJsonArtifact('BLACK_LEDGER_COMMAND_MAP_ACCESSIBILITY.generated.json', {
    status: 'pass',
    base_url: baseUrl,
    route: '/ledger/map',
    list_fallback_cards: await page.locator('.ledger-world-shell__districts [data-map-district-card]').count(),
  });
});
