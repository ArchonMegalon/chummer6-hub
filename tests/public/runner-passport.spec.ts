...
test('runner passport public route stays available and points at the signed-in continuity bench', async ({ request, page }) => {
  ...
  expect(payload.publicBoard.runnerReturnMarkdownHref).toBe('/passport/receipts/runner_return_posture.md');
  expect(payload.publicBoard.runnerReturnJsonHref).toBe('/passport/receipts/runner_return_posture.json');
  ...
  await expect(page.locator('body')).toContainText('Runner Passport now ships a real first-party continuity lane');
  await expect(page.locator('body')).toContainText('Sign in for Runner Passport');
  await expect(page.locator('body')).toContainText('Private identity links, moderation state, and account recovery stay signed-in.');
