...
def test_source_bundle_materializer_fails_visual_proof_before_persistent_publication() -> None:
    source = (REPO_ROOT / "scripts" / "materialize-public-downloads-bundle.sh").read_text(
        encoding="utf-8"
    )
    status_capture = source.index("windows_visual_proof_gate_status=$?")
    guard = source.index(
    ...
    fail_closed_block = source[guard:first_persistent_write]

    assert status_capture < guard < first_persistent_write
    assert "refresh_windows_visual_proof_operator_receipts" in fail_closed_block
    assert '"$PUBLIC_RELEASE_CHANNEL_SOURCE_PATH"' in fail_closed_block
    assert '"$OUTPUT_ROOT"' in fail_closed_block
    assert 'exit "$windows_visual_proof_gate_status"' in fail_closed_block
    assert source.count(
        'if [[ "${windows_visual_proof_gate_status:-0}" -ne 0 ]]; then'
    ) == 1


def test_source_bundle_materializer_repairs_only_manifest_matching_artifact_collisions() -> None:
    source = (REPO_ROOT / "scripts" / "materialize-public-downloads-bundle.sh").read_text(
        encoding="utf-8"
    )
    hydrate_start = source.index("def ensure_manifest_owned_file(")
    hydrate_end = source.index("\ntarget_root = Path(sys.argv[1])", hydrate_start)
    hydrate = source[hydrate_start:hydrate_end]

    assert "target_path.is_file() and file_matches(target_path, expected_sha)" in hydrate
    assert "return target_root" in hydrate
    assert "match = find_matching_file(candidate_roots, file_name, expected_sha)" in hydrate
    assert "if match is None:" in hydrate
    assert "if source_path.resolve() != target_path.resolve():" in hydrate
    assert "replace_file(source_path, target_path)" in hydrate
    assert '"$PRESENTATION_ROOT/Chummer.Portal/downloads/files"' in source
    assert '"$PRESENTATION_ROOT/Docker/Downloads/files"' in source


def test_source_bundle_materializer_gates_candidate_evidence_without_restamping_or_synthesis() -> None:
    source = (REPO_ROOT / "scripts" / "materialize-public-downloads-bundle.sh").read_text(
        encoding="utf-8"
    )
    ...
    assert '"$PUBLIC_RELEASE_CHANNEL_SOURCE_PATH"' in stage_block
    assert '"$PRESENTATION_RELEASE_EVIDENCE_SOURCE"' in stage_block
    assert '"$DISABLED_ARTIFACT_IDS"' in stage_block
    assert 'stage-release-candidate-evidence.py' in source

    publish_start = source.index('rm -rf "$OUTPUT_ROOT/startup-smoke"')
    publish_end = source.index('\nPY\n\nif [[ -n "$DISABLED_ARTIFACT_IDS" ]]', publish_start)
    ...
    for forbidden_assignment in (
        ...
        assert forbidden_assignment not in publish_block
    assert "subprocess.check_output" not in publish_block
    assert 'git", "show"' not in publish_block
    assert "write_bytes(b\"\")" not in publish_block
    assert "refusing to synthesize or restore it" in publish_block
