...
SCRIPT = REPO_ROOT / "scripts" / "verify_next90_m128_hub_privacy_bounded_support_status.py"
SOURCE_FILES = [
...
class Next90M128HubPrivacyBoundedSupportStatusTests(unittest.TestCase):
    def test_verifier_accepts_repo_local_privacy_bounded_support_lane(self) -> None:
        ...
        self.assertIn("next90 m128 hub privacy bounded support status proof passed", result.stdout)

    def test_verify_script_runs_m128_guard(self) -> None:
        verify_script = (REPO_ROOT / "scripts" / "ai" / "verify.sh").read_text(encoding="utf-8")
        self.assertIn("python3 scripts/materialize_next90_m128_hub_privacy_bounded_support_status_proof.py", verify_script)
        self.assertIn("python3 scripts/verify_next90_m128_hub_privacy_bounded_support_status.py", verify_script)
        self.assertIn("python3 -m unittest tests/test_next90_m128_hub_privacy_bounded_support_status.py", verify_script)

    def test_verifier_fails_when_queue_row_drifts(self) -> None:
        with tempfile.TemporaryDirectory(prefix="next90-m128-queue-") as temp_dir:
            ...
            for item in queue_payload["items"]:
                if item.get("package_id") == "next90-m128-hub-implement-privacy-bounded-support-crash-feedback-telemet":
                    item["frontier_id"] = 0
                    break
        ...
        self.assertIn("frontier_id must be 4025965979", result.stderr)

    def test_verifier_fails_when_smoke_loses_retention_assertion(self) -> None:
        with tempfile.TemporaryDirectory(prefix="next90-m128-smoke-") as temp_dir:
            temp_root = Path(temp_dir)
            self.copy_sources(temp_root)
            smoke_path = temp_root / "tests/RunServicesSmoke/Program.cs"
            smoke_text = smoke_path.read_text(encoding="utf-8")
            smoke_path.write_text(smoke_text.replace("retention clocks on the privacy boundary route", "retention drift"), encoding="utf-8")
            result = self.run_verifier(temp_root)

        self.assertNotEqual(result.returncode, 0)
        self.assertIn("retention clocks on the privacy boundary route", result.stderr)
    ...
    def run_verifier(self, temp_root: Path, *, queue_path: Path | None = None, design_queue_path: Path | None = None) -> subprocess.CompletedProcess[str]:
        env = os.environ.copy()
        env["CHUMMER_NEXT90_M128_ROOT"] = str(temp_root)
        if queue_path is not None:
            env["CHUMMER_NEXT90_M128_QUEUE_STAGING"] = str(queue_path)
        if design_queue_path is not None:
            env["CHUMMER_NEXT90_M128_DESIGN_QUEUE_STAGING"] = str(design_queue_path)
        return subprocess.run(["python3", str(temp_root / "scripts/verify_next90_m128_hub_privacy_bounded_support_status.py")], cwd=temp_root, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def load_queue_payload(path: Path) -> dict:
        ...
        package_marker = "package_id: next90-m128-hub-implement-privacy-bounded-support-crash-feedback-telemet"
