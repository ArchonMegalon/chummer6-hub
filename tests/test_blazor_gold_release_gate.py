...
class BlazorGoldReleaseGateTests(unittest.TestCase):
    ...
    def test_release_ready_script_only_requires_blazor_gold_when_public_release_requires_blazor(self) -> None:
        text = RELEASE_READY_SCRIPT.read_text(encoding="utf-8")
        self.assertIn('if [[ "${CHUMMER_PUBLIC_REQUIRE_BLAZOR:-0}" =~ ^(1|true|yes|on)$ ]]; then', text)
        self.assertIn('gates+=("verify_chummer6_blazor_gold:bash $root/scripts/release/verify_chummer6_blazor_gold.sh")', text)
