...
SCRIPT = REPO_ROOT / "scripts" / "verify_next90_m129_hub_reusable_account_flows.py"
SOURCE_FILES = [
...
class Next90M129HubReusableAccountFlowsTests(unittest.TestCase):
    def test_verifier_accepts_repo_local_reusable_account_flow_lane(self) -> None:
        ...
        self.assertIn("next90 m129 hub reusable account flows proof passed", result.stdout)

    def test_verify_script_runs_m129_guard(self) -> None:
        verify_script = (REPO_ROOT / "scripts" / "ai" / "verify.sh").read_text(encoding="utf-8")
        self.assertIn("python3 scripts/materialize_next90_m129_hub_reusable_account_flows_proof.py", verify_script)
        self.assertIn("python3 scripts/verify_next90_m129_hub_reusable_account_flows.py", verify_script)
        self.assertIn("python3 -m unittest tests/test_next90_m129_hub_reusable_account_flows.py", verify_script)

    def test_verifier_fails_when_queue_row_drifts(self) -> None:
        with tempfile.TemporaryDirectory(prefix="next90-m129-queue-") as temp_dir:
            ...
            for item in queue_payload["items"]:
                if item.get("package_id") == "next90-m129-hub-build-reusable-account-profile-group-membership-join-cod":
                    item["frontier_id"] = 0
                    break
        ...
        self.assertIn("frontier_id must be 1246056730", result.stderr)

    def test_verifier_fails_when_smoke_loses_entitlement_assertion(self) -> None:
        with tempfile.TemporaryDirectory(prefix="next90-m129-smoke-") as temp_dir:
            temp_root = Path(temp_dir)
            self.copy_sources(temp_root)
            smoke_path = temp_root / "tests/RunServicesSmoke/Program.cs"
            smoke_text = smoke_path.read_text(encoding="utf-8")
            smoke_path.write_text(smoke_text.replace("entitlement-journal followthrough on the signed-in entitlements rail", "entitlement drift"), encoding="utf-8")
            result = self.run_verifier(temp_root)

        self.assertNotEqual(result.returncode, 0)
        self.assertIn("entitlement-journal followthrough on the signed-in entitlements rail", result.stderr)
    ...
    def run_verifier(self, temp_root: Path, *, queue_path: Path | None = None, design_queue_path: Path | None = None) -> subprocess.CompletedProcess[str]:
        env = os.environ.copy()
        env["CHUMMER_NEXT90_M129_ROOT"] = str(temp_root)
        if queue_path is not None:
            env["CHUMMER_NEXT90_M129_QUEUE_STAGING"] = str(queue_path)
        if design_queue_path is not None:
            env["CHUMMER_NEXT90_M129_DESIGN_QUEUE_STAGING"] = str(design_queue_path)
        return subprocess.run(["python3", str(temp_root / "scripts/verify_next90_m129_hub_reusable_account_flows.py")], cwd=temp_root, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def load_queue_payload(path: Path) -> dict:
        ...
        package_marker = "package_id: next90-m129-hub-build-reusable-account-profile-group-membership-join-cod"
