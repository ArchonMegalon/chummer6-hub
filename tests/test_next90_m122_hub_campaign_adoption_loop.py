...
SCRIPT = REPO_ROOT / 'scripts' / 'verify_next90_m122_hub_campaign_adoption_loop.py'
SOURCE_FILES = [
...
class Next90M122HubCampaignAdoptionLoopTests(unittest.TestCase):
    def test_verifier_accepts_repo_local_campaign_adoption_loop_lane(self) -> None:
        ...
        self.assertIn("next90 m122 hub campaign adoption loop proof passed", result.stdout)

    def test_verify_script_runs_m122_guard(self) -> None:
        verify_script = (REPO_ROOT / 'scripts' / 'ai' / 'verify.sh').read_text(encoding='utf-8')
        self.assertIn('python3 scripts/materialize_next90_m122_hub_campaign_adoption_loop_proof.py', verify_script)
        self.assertIn('python3 scripts/verify_next90_m122_hub_campaign_adoption_loop.py', verify_script)
        self.assertIn('python3 -m unittest tests/test_next90_m122_hub_campaign_adoption_loop.py', verify_script)
    ...
    def test_verifier_fails_when_queue_row_drifts(self) -> None:
        ...
        self.assertIn('frontier_id must be 1630681972', result.stderr)

    def test_verifier_fails_when_smoke_loses_adoption_loop_endpoint(self) -> None:
        with tempfile.TemporaryDirectory(prefix='next90-m122-smoke-') as temp_dir:
            ...
            smoke_path.write_text(smoke_text.replace('GetMyCampaignWorkspaceAdoptionLoop', 'GetMyCampaignWorkspaceAdoptionLoopDrift'), encoding='utf-8')
            result = self.run_verifier(temp_root)

        self.assertNotEqual(result.returncode, 0)
        self.assertIn('GetMyCampaignWorkspaceAdoptionLoop', result.stderr)
    ...
    def run_verifier(self, temp_root: Path, *, queue_path: Path | None = None, design_queue_path: Path | None = None) -> subprocess.CompletedProcess[str]:
        env = os.environ.copy()
        env['CHUMMER_NEXT90_M122_ROOT'] = str(temp_root)
        if queue_path is not None:
            env['CHUMMER_NEXT90_M122_QUEUE_STAGING'] = str(queue_path)
        if design_queue_path is not None:
            env['CHUMMER_NEXT90_M122_DESIGN_QUEUE_STAGING'] = str(design_queue_path)
        return subprocess.run(["python3", str(temp_root / 'scripts/verify_next90_m122_hub_campaign_adoption_loop.py')], cwd=temp_root, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def load_queue_payload(path: Path) -> dict:
        ...
        package_marker = 'package_id: next90-m122-hub-implement-campaign-adoption-wizard-state-runner-goal-per'
