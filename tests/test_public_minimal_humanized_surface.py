...
def test_participation_surface_renders_first_party_without_character_helper_copy() -> None:
    ...
    assert '<h1 id="partizipate-title" class="sr-only">Participate</h1>' in participate
    ...
    assert "@PublicParticipateText(Model.Heading)" not in participate
    assert "@PublicParticipateText(Model.Summary)" not in participate
...
def test_downloads_surface_requires_full_stable_release_tuple_before_populating_stable_card() -> None:
    downloads = read("Chummer.Run.Api/Views/PublicLanding/Downloads.cshtml")

    assert "var isStableRelease = IsPublicStableRelease(Model.Manifest);" in downloads
    ...
    assert '|| string.Equals(channel, "stable", StringComparison.OrdinalIgnoreCase)' in downloads
    assert '|| string.Equals(channel, "docker", StringComparison.OrdinalIgnoreCase)' in downloads
    assert "var releaseNeedsReview = NeedsReview(Model.Manifest) && !IsPublicStableRelease(Model.Manifest);" not in downloads
    assert "var stable = releaseNeedsReview || requestedPlatformUnavailable ? null : recommended;" not in downloads


def test_status_surface_uses_explicit_release_posture_labels() -> None:
    status = read("Chummer.Run.Api/Views/PublicLanding/Status.cshtml")

    assert 'ViewData["Title"] = "Status";' in status
    assert "Preview downloads" in status
    assert "Stable downloads" in status
    assert "Downloads paused" in status
    assert "@PublicStatusText(Model.ReleaseSummary)" in status
    assert "@PublicStatusText(Model.CautionSummary)" in status
    assert "<h1>Updated</h1>" not in status
    assert "data-downloads-release-version" in status


def test_status_surface_requires_published_gold_release_before_stable_copy() -> None:
    status = read("Chummer.Run.Api/Views/PublicLanding/Status.cshtml")

    assert "static bool IsPublishedStableRelease(PublicReleaseManifestDto manifest)" in status
    assert 'string.Equals((manifest.SupportabilityState ?? string.Empty).Trim(), "gold_supported", StringComparison.OrdinalIgnoreCase)' in status
    assert 'string.Equals((manifest.Status ?? string.Empty).Trim(), "published", StringComparison.OrdinalIgnoreCase)' in status
    assert "var isStableRelease = IsPublishedStableRelease(Model.Manifest);" in status
    assert '|| string.Equals((Model.Manifest.SupportabilityState ?? string.Empty).Trim(), "gold_supported", StringComparison.OrdinalIgnoreCase)' not in status


def test_public_status_controller_requires_published_gold_release_before_stable_summary() -> None:
    controller = read("Chummer.Run.Api/Controllers/PublicLandingController.cs")

    assert "private static bool IsPublishedStableRelease(PublicReleaseManifestDto manifest)" in controller
    assert 'if (IsPublishedStableRelease(manifest))' in controller
    assert 'return string.Equals((manifest.Status ?? string.Empty).Trim(), "published", StringComparison.OrdinalIgnoreCase);' in controller
    assert '|| string.Equals((manifest.Channel ?? string.Empty).Trim(), "docker", StringComparison.OrdinalIgnoreCase)' in controller
    assert 'string asOf = $"As of {BuildLiveVerificationLabel(manifest)}."; '[:-1] in controller
    assert 'string checkedAt = $"Checked {BuildLiveVerificationLabel(manifest)}."; '[:-1] not in controller
    assert 'if (stableLanePublished || string.Equals(supportabilityState, "gold_supported", StringComparison.OrdinalIgnoreCase))' not in controller
