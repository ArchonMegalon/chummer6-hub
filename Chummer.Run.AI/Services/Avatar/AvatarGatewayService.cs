    ...
    int maximumCachedOperations = 8_192) : IAvatarGatewayService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        lock (_gate)
        {
            ...
            else
            {
                ...
                AvatarRuleAuthorityBinding? binding = ruleAuthorityClient.Binding;
                AvatarRuleAuthorityRequest authorityRequest = BuildAuthorityRequest(
                    authorization.Context,
                    request,
                    payloadDigest,
                    binding);
                answerTask = HasRuleCharacterScopes(authorization.Context) && binding is not null
                    ? ResolveRuleCoreAsync(authorityRequest, lifetime.Cancellation.Token)
                    : Task.FromResult(HasRuleCharacterScopes(authorization.Context)
                        ? AvatarRuleAnswerValidator.SafeUnavailable(
                            authorityRequest,
                            "core-typed-rule-authority-binding-unavailable")
                        : SafeScopeFailure(authorityRequest));
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            AvatarRuleAnswerEnvelope answer = await ruleAuthorityClient
            ...
            return failures.Count == 0
                ? answer
                : AvatarRuleAnswerValidator.SafeUnavailable(request);
        }
        catch (AvatarRuleAuthorityException exception)
        {
            return exception.StatusCode switch
            {
                StatusCodes.Status409Conflict => SafeAuthorityConflict(request),
                StatusCodes.Status410Gone => SafeAuthorityStale(request),
                _ => AvatarRuleAnswerValidator.SafeUnavailable(request)
            };
        }
    ...
    private static AvatarRuleAnswerEnvelope SafeAuthorityConflict(
        ...
        => AvatarRuleAnswerValidator.SafeFailure(
            ...
            "chummer-core-rule-authority-conflict");
    ...
    private static AvatarRuleAnswerEnvelope SafeAuthorityStale(
        ...
        => AvatarRuleAnswerValidator.SafeFailure(
            ...
            "chummer-core-rule-authority-stale");
        ...
        AvatarRuleAuthorityBinding? binding)
    {
        AvatarRuleAuthorityRequest unsigned = new(
            ...
            gatewayOperationDigest,
            binding?.ContractName ?? string.Empty,
            binding?.CorePackageId ?? string.Empty,
            binding?.CorePackageVersion ?? string.Empty,
            binding?.CorePackageDigest ?? string.Empty,
            context.Locale,
            request.Question,
            request.SubjectId,
            string.Empty);
