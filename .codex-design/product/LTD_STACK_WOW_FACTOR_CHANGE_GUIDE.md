...
Chummer already owns enough bounded external tools to ship a memorable, differentiated **concierge + artifact factory** experience.

The immediate problem is not missing tools.
The immediate problem is that the design and inventory have drifted:
- `FacePop`, `Lunacal`, and `Deftform` are now owned but are not yet represented in the current design inventory.
- the current blocking rule is too blunt for low-risk public trust widgets.
- the media and support stacks are strong, but there is no first-class **public concierge** layer that turns those tools into obvious workflows.
...
Add:
- `FacePop` as **Bounded** public trust / concierge widget lane
...
- CTA 2: which platform should I use? -> vidBoard explainer + article fallback
- CTA 3: I need setup help -> Deftform support-enrichment form
- CTA 4: book a setup clinic -> Lunacal booking
- Email/confirmation -> Hub + optional Emailit
...
- submit concept/questions -> Deftform
...
- book help -> Lunacal
- after successful fix/use -> FacePop testimonial capture
...
Goal:
Give runsite/location artifacts a premium guided feel.
...
Recipe:
- optional FacePop response capture asking: what landed, what dragged, what should continue?
- Hedy and Nonverbia remain the structured/debrief analysis lanes
...
- add FacePop/Lunacal/Deftform references to public trust and guide policy where appropriate
...
- store publication references for approved testimonial/public proof artifacts
