...
* `knowledge-fabric` - `Prompting Systems`, `Documentation.AI`, `AI Magicx`, `1min.AI`, `BrowserAct`, `Blip AI`, and `Paperguide` may build, audit, capture, dictate, or cite derived projections, but core-owned rules and receipts remain authoritative
...
* `runbook-press` - long-form authoring and export may use `First Book ai`, `MarkupGo`, `Documentation.AI`, and `Blip AI`; campaign primer and module explainer videos may use `vidBoard`; narrated companion assets may use `Soundmadeseen`; bounded candidate voice or cited research support may use `Unmixr AI` and `Paperguide`
