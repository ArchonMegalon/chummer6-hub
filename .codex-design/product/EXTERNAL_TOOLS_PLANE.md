...
* `knowledge-fabric` - `Prompting Systems`, `Documentation.AI`, `AI Magicx`, `1min.AI`, `BrowserAct`, and `Paperguide` may build, audit, capture, or cite derived projections, while `Blip AI` may assist bounded dictation, transcript cleanup, and operator draft capture for research, help, and synthesis packets only; core-owned rules and receipts remain authoritative
...
* `runbook-press` - long-form authoring and export may use `First Book ai`, `MarkupGo`, and `Documentation.AI`; campaign primer and module explainer videos may use `vidBoard`; narrated companion assets may use `Soundmadeseen`; bounded candidate voice or cited research support may use `Unmixr AI` and `Paperguide`, while `Blip AI` may assist bounded dictation, cleanup, and draft capture for operator-authoring packets only; accepted content still lands in Chummer-owned source
