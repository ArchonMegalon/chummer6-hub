...
- human booking/escalation (`Lunacal`)
...
- no first-class **public concierge / trust widget** model
- no machine-readable workflow family for **FacePop -> Deftform -> Lunacal -> Hub** handoff
- no first-class testimonial or video-response capture lane
- no policy exception for **public, low-risk trust widgets** on Hub-owned pages
- current tool inventory and EA inventory lag recently acquired tools (`FacePop`, `Lunacal`, `Deftform`)
...
- crash/bug submission forms themselves
...
- collect lightweight opt-in lead/support/testimonial data
- collect video/audio responses for moderated testimonial intake
- wrap a booking or intake call to action with human context

Must not:
- answer support questions as truth
- become support ticket truth
- become install/update truth
- become auth/account truth
- appear as the desktop/mobile support brain
...
- GM office hours
- creator consults
- migration/setup clinics
- premium/publisher review sessions
...
- campaign join/application forms
...
- become the only submission path for crash/bug reporting
...
Entry:
- `/now`, release pages, signed-in help entry pages
...
5. Setup/update difficulty routes to Deftform and, if needed, Lunacal.
...
4. ApproveThis or Hub-owned moderation UI approves publication.
...
3. AvoMap and Crezlo Tours provide route/explorable modes.
