...
* guest access to `/participate/codex` must fall back to `/auth/google/start?next=/participate/codex` while `/login` is an allowed fallback only when no auth provider route is available
