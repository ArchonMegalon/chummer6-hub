...
* guest access to `/participate/codex` must redirect or fall back to `/auth/google/start?next=/participate/codex` while a direct `/login` fallback is allowed if the configured provider route is unavailable
