...
public sealed class PublicSignalOperationsServiceTests
{
    ...
    public void BuildPacketDefaultsToFirstPartyWhenHostedPromotionIsMissing()
    {
        ...
        Assert.Contains("Recipient matching is still missing", packet.RecipientProjectionSummary, StringComparison.Ordinal);
        Assert.Contains("The outbox stays blocked", packet.QueueSummary, StringComparison.Ordinal);
        Assert.Contains("Outbox creation stays blocked", packet.ReleaseProofSummary, StringComparison.Ordinal);
        Assert.DoesNotContain("Hub", packet.RecipientProjectionSummary, StringComparison.Ordinal);
        Assert.DoesNotContain("return rail", packet.RecipientProjectionSummary, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("Hub outbox", packet.QueueSummary, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("Outbox materialization", packet.ReleaseProofSummary, StringComparison.OrdinalIgnoreCase);
    ...
    public void RecordWebhookMaterializesBoundedReceiptMetadataAndDeduplicatesRedelivery()
    {
        ...
        Assert.Equal("Recipient list pending", packet.RecentCloseoutReceipts[0].StatusLabel);
        ...
        Assert.Contains("recipient matching", packet.RecentCloseoutReceipts[0].DeliveryReason, StringComparison.OrdinalIgnoreCase);
        ...
        Assert.Equal("Recipient list pending", packet.RecentQueueReceipts[0].StatusLabel);
        ...
        Assert.Contains("recipient matching", packet.RecentQueueReceipts[0].QueueReason, StringComparison.OrdinalIgnoreCase);
    ...
    public void FullCloseoutRuntimeReadinessTurnsShippedWebhookIntoDeliveryCandidate()
    {
        ...
        Assert.Equal("Recipient list ready", packet.RecipientProjectionStatusLabel);
        ...
        Assert.Equal("hub_preferences:follow_horizons", packet.RecentCloseoutReceipts[0].ConsentSourceRef);
        Assert.Contains("closeout approval", packet.RecentCloseoutReceipts[0].DeliveryReason, StringComparison.OrdinalIgnoreCase);
