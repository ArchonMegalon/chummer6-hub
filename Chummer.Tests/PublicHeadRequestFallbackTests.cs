...
public sealed class PublicHeadRequestFallbackTests
{
    ...
    public void PublicEdgeHealthEndpointAcceptsHeadExplicitly()
    {
        ...
        Assert.Contains("app.MapMethods(\"/mobile/service-worker.js\", new[] { HttpMethods.Get, HttpMethods.Head }", program, StringComparison.Ordinal);
        Assert.Contains("context.Response.Headers.CacheControl = \"no-cache, no-store, must-revalidate\";", program, StringComparison.Ordinal);
        Assert.Contains("? \"public, max-age=300, must-revalidate\"", program, StringComparison.Ordinal);
        Assert.Contains(": \"public, max-age=14400, must-revalidate\";", program, StringComparison.Ordinal);
