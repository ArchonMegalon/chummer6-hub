...
public sealed class BlackLedgerTickNewsTests
{
    ...
    public void BlackLedgerTickNews_routes_and_views_exist()
    {
        ...
        Assert.Contains("Open advisory lane", notificationsView, StringComparison.Ordinal);
        Assert.Contains("LedgerAdvisory.cshtml", controller, StringComparison.Ordinal);
        Assert.Contains("World-tick validation", File.ReadAllText(RepoPaths.FromRoot("Chummer.Run.Api", "Views", "PublicLanding", "LedgerWorldTickValidation.cshtml")), StringComparison.Ordinal);
