...
public sealed class ReleaseBundlePromotionServiceTests
{
    ...
    [Fact]
    public async Task PromoteAsyncGenerationBindsNestedReleaseProofLookalike()
    {
        using var fixture = new ReleaseBundlePromotionFixture();
        const string artifactId = "avalonia-win-x64-installer";
        string bundlePath = fixture.CreateBundle(
        ...
        Assert.Equal($"/downloads/g/{result.GenerationId}/install/{artifactId}", nestedRoute);
    }
        ...
        Task reader = Task.Run(() =>
        {
            while (!stopReaders.IsCancellationRequested)
            {
                try
                {
                    ...
                    Assert.Equal(generationId, manifest.RootElement.GetProperty("generationId").GetString());
                    ...
                    Assert.StartsWith($"/downloads/g/{generationId}/install/", downloadUrl, StringComparison.Ordinal);
    ...
    [Fact]
    public async Task RepairCompatibilityMirrorsRestoresActiveGenerationWithoutChangingPointer()
    {
        using var fixture = new ReleaseBundlePromotionFixture();
        const string artifactId = "avalonia-win-x64-installer";
        const string fileName = "chummer-avalonia-win-x64-installer.exe";
        string bundle = fixture.CreateBundle(
            version: "run-20260715-mirror-repair",
            artifacts:
            [
                new BundleArtifact(
                    artifactId, "avalonia", "windows", "x64", "installer", fileName,
                    "mirror-source"u8.ToArray(), false, false, "skipped_preview")
            ]);
        ReleaseBundlePromotionResult promoted = await fixture.PromoteAsync(bundle);
        string generationRoot = Path.Combine(
            fixture.DownloadsRoot,
            "generations",
            promoted.GenerationId!);
        IReadOnlyDictionary<string, byte[]> generationBefore = SnapshotTree(generationRoot);
        byte[] pointerBefore = File.ReadAllBytes(Path.Combine(fixture.DownloadsRoot, "current.json"));
        ...
        File.WriteAllText(
            ...
            """{"version":"corrupt"}""");
        File.WriteAllText(
            Path.Combine(fixture.DownloadsRoot, "files", fileName),
            "corrupt");
        Directory.CreateDirectory(Path.Combine(fixture.DownloadsRoot, "proof"));
        File.WriteAllText(
            Path.Combine(fixture.DownloadsRoot, "proof", "stale.receipt.json"),
            "{}");

        ReleaseCompatibilityMirrorRepairResult repair =
            fixture.RepairCompatibilityMirrors();

        Assert.Equal(promoted.GenerationId, repair.GenerationId);
        Assert.Equal("run-20260715-mirror-repair", repair.ReleaseVersion);
        Assert.Equal("preview", repair.Channel);
        Assert.True(repair.MirroredFileCount >= 3);
        Assert.Equal(
            File.ReadAllBytes(Path.Combine(generationRoot, "releases.json")),
            File.ReadAllBytes(Path.Combine(fixture.DownloadsRoot, "releases.json")));
        Assert.Equal(
            File.ReadAllBytes(Path.Combine(generationRoot, "RELEASE_CHANNEL.generated.json")),
            File.ReadAllBytes(Path.Combine(fixture.DownloadsRoot, "RELEASE_CHANNEL.generated.json")));
        Assert.Equal(
            File.ReadAllBytes(Path.Combine(generationRoot, "files", fileName)),
            File.ReadAllBytes(Path.Combine(fixture.DownloadsRoot, "files", fileName)));
        Assert.False(File.Exists(
            Path.Combine(fixture.DownloadsRoot, "proof", "stale.receipt.json")));
        Assert.Equal(pointerBefore, File.ReadAllBytes(Path.Combine(fixture.DownloadsRoot, "current.json")));
        AssertManagedShelfMatches(generationBefore, SnapshotTree(generationRoot));
    }

    [Fact]
    public async Task RepairCompatibilityMirrorsRejectsLinkedTargetWithoutFollowingIt()
    {
        if (OperatingSystem.IsWindows())
        {
            return;
        ...
        const string fileName = "chummer-avalonia-win-x64-installer.exe";
        string bundle = fixture.CreateBundle(
            version: "run-20260715-mirror-link",
            artifacts:
            [
                new BundleArtifact(
                    "avalonia-win-x64-installer", "avalonia", "windows", "x64", "installer", fileName,
                    "mirror-source"u8.ToArray(), false, false, "skipped_preview")
            ]);
        _ = await fixture.PromoteAsync(bundle);
        byte[] pointerBefore = File.ReadAllBytes(Path.Combine(fixture.DownloadsRoot, "current.json"));
        string externalRoot = Path.Combine(
            Path.GetTempPath(),
            $"release-mirror-external-{Guid.NewGuid():N}");
        Directory.CreateDirectory(externalRoot);
        string sentinel = Path.Combine(externalRoot, "sentinel.txt");
        File.WriteAllText(sentinel, "outside");
        try
        {
            Directory.Delete(Path.Combine(fixture.DownloadsRoot, "files"), recursive: true);
            Directory.CreateSymbolicLink(
                Path.Combine(fixture.DownloadsRoot, "files"),
                externalRoot);
        ...
        {
            string linkedPath = Path.Combine(fixture.DownloadsRoot, "files");
    ...
    [Fact]
    public async Task PromoteAsyncAllowsExplicitCoverageIncompletePreviewWineCompatibilityReceipt()
    {
        using var fixture = new ReleaseBundlePromotionFixture();
        string bundlePath = fixture.CreateBundle(
            version: "run-20260717-wine-preview-coverage-incomplete",
            artifacts:
            [
                new BundleArtifact(
                    ArtifactId: "avalonia-win-x64-installer",
                    ...
                    Kind: "installer",
                    FileName: "chummer-avalonia-win-x64-installer.exe",
                    Bytes: "wine-preview-compatibility"u8.ToArray(),
                    RequiresSigning: false,
                    RequiresNotarization: false,
                    SigningStatusOverride: "skipped_preview")
            ],
        ...
        foreach (string manifestName in new[] { "releases.json", "RELEASE_CHANNEL.generated.json" })
        {
            fixture.RewriteBundleManifest(
                bundlePath,
                manifestName,
                manifest => manifest["rolloutState"] = "coverage_incomplete");
        }
        fixture.RewriteOnlyStartupSmokeReceipt(
            bundlePath,
            receipt =>
            {
                receipt["executionEnvironment"] = "wine_compatibility";
                receipt["verificationScope"] = "windows_compatibility_startup";
                receipt["nativeHostEvidence"] = new JsonObject
                {
                    ["contractName"] = "chummer6-ui.native_windows_host_evidence",
                    ["status"] = "not_native",
        ...
        fixture.RewriteBundleManifest(
            ...
            evidence =>
            {
                JsonObject row = evidence["artifacts"]![0]!.AsObject();
                row["nativeWindowsStartupProofRequired"] = false;
                row["startupSmokeExecutionEnvironment"] = "wine_compatibility";
            });

        ReleaseBundlePromotionResult result = await fixture.PromoteAsync(bundlePath);

        Assert.Equal("run-20260717-wine-preview-coverage-incomplete", result.Version);
    }
    ...
    public async Task PromoteAsyncMarksPromotedPrimaryInstallerAsReinstallRecoveryWhenFallbackIsMissing()
    {
        ...
        Assert.Equal(3, releaseChannel.GetProperty("recommendedRouteCount").GetInt32());
        ...
        Assert.Equal(3, boundaryReleaseChannel.GetProperty("promotedInstallerTupleCount").GetInt32());
        Assert.Equal("live", boundaryReleaseChannel.GetProperty("publicTrustPosture").GetString());
    }

    [Fact]
    public async Task PromoteAsyncRebuildsPublicTrustMetricsFromNormalizedRouteTruth()
    {
        using var fixture = new ReleaseBundlePromotionFixture();
        ...
        int expectedPrimary = rows.Count(row =>
            row.GetProperty("routeRole").GetString() == "primary"
            && row.GetProperty("promotionState").GetString() == "promoted"
            && row.GetProperty("revokeState").GetString() != "revoked");
        int expectedFallback = rows.Count(row =>
            row.GetProperty("routeRole").GetString() == "fallback"
            && row.GetProperty("promotionState").GetString() == "promoted"
            && row.GetProperty("revokeState").GetString() != "revoked");
        int expectedBlocked = rows.Count(row =>
            row.GetProperty("promotionState").GetString() == "proof_required"
            && row.GetProperty("revokeState").GetString() != "revoked"
            && !(
                row.GetProperty("routeRole").GetString() == "fallback"
                && row.GetProperty("parityPosture").GetString() == "explicit_fallback"));
        int expectedRevoked = rows.Count(row =>
            row.GetProperty("revokeState").GetString() == "revoked"
            || row.GetProperty("promotionState").GetString() == "revoked");
        ...
        Assert.Equal(expectedBlocked, releaseChannel.GetProperty("blockedRouteCount").GetInt32());
        Assert.Equal(expectedRevoked, releaseChannel.GetProperty("revokedRouteCount").GetInt32());
        Assert.Equal(expectedPrimary, adoptionHealth.GetProperty("primaryPromotedCount").GetInt32());
        Assert.Equal(expectedFallback, adoptionHealth.GetProperty("fallbackRecoveryCount").GetInt32());
        Assert.Equal(expectedBlocked, adoptionHealth.GetProperty("blockedRouteCount").GetInt32());
        Assert.Equal(expectedRevoked, adoptionHealth.GetProperty("revokedRouteCount").GetInt32());
        Assert.Equal(expectedRevoked, revocationFacts.GetProperty("activeRevocationCount").GetInt32());
        Assert.Equal(expectedRevoked, revocationFacts.GetProperty("activeRevocations").GetArrayLength());
        Assert.DoesNotContain("stale", releaseChannel.GetProperty("summary").GetString(), StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("stale", adoptionHealth.GetProperty("summary").GetString(), StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("stale", revocationFacts.GetProperty("summary").GetString(), StringComparison.OrdinalIgnoreCase);
    }
    ...
    [Fact]
    public async Task PromoteAsyncDowngradesPassedReleaseProofWhenTimestampIsTooFarAfterPublication()
    {
        using var fixture = new ReleaseBundlePromotionFixture();
    ...
    public async Task PromoteAsyncEvaluatesFreshnessFactsAtDeterministicCommitInstant(
        string scenario,
        ...
        int evaluationOffsetSeconds)
    {
        ...
        long ageSeconds = 0;
        object publicTrustMetrics = scenario == "bare"
            ? new Dictionary<string, object?>
            {
                ["proofFreshness"] = new Dictionary<string, object?> { ["status"] = "fresh" }
            }
    ...
    [Fact]
    public async Task PromotionAcceptsExactRegistryCanonicalIncomingUrlsAndNormalizesThem()
    {
        using var fixture = new ReleaseBundlePromotionFixture();
        const string artifactId = "avalonia-win-x64-installer";
        const string payloadFileName = "chummer-avalonia-win-x64-payload.zip";
        string bundlePath = fixture.CreateBundle(
            version: "run-20260715-canonical-incoming-url",
            artifacts:
            [
                new BundleArtifact(
                    ArtifactId: artifactId,
        ...
        foreach (string manifestName in new[] { "releases.json", "RELEASE_CHANNEL.generated.json" })
        {
            ...
            {
                JsonObject row = root[manifestName == "releases.json" ? "downloads" : "artifacts"]![0]!.AsObject();
                string urlField = manifestName == "releases.json" ? "url" : "downloadUrl";
                row[urlField] = $"https://chummer.run{row[urlField]!.GetValue<string>()}";
                row["payloadDownloadUrl"] =
                    $"https://chummer.run{row["payloadDownloadUrl"]!.GetValue<string>()}";
                row["payloadAcquisitionMode"] = "download";
                if (manifestName == "releases.json")
                {
                    row["platformLabel"] = row["platform"]!.DeepClone();
                    row["platform"] = "windows";
                }
            });
        }
        JsonObject sidecar = JsonNode.Parse(
            fixture.ReadBundleEntry(bundlePath, $"files/{payloadFileName}.json"))!.AsObject();
        sidecar["downloadUrl"] =
            $"https://chummer.run{sidecar["downloadUrl"]!.GetValue<string>()}";
        sidecar["payloadAcquisitionMode"] = "download";
        fixture.ReplaceBundleEntry(
            bundlePath,
            $"files/{payloadFileName}.json",
            Encoding.UTF8.GetBytes(sidecar.ToJsonString(TestJsonOptions)));

        ReleaseBundlePromotionResult result = await fixture.PromoteAsync(bundlePath);

        string expectedInstaller =
            $"/downloads/g/{result.GenerationId}/install/{artifactId}";
        string expectedPayload = expectedInstaller + "/payload";
        using JsonDocument canonical = fixture.ReadGenerationJson(
            result.GenerationId!,
            "RELEASE_CHANNEL.generated.json");
        JsonElement promoted = canonical.RootElement.GetProperty("artifacts")[0];
        Assert.Equal(expectedInstaller, promoted.GetProperty("downloadUrl").GetString());
        Assert.Equal(expectedPayload, promoted.GetProperty("payloadDownloadUrl").GetString());
    }
    ...
    private sealed class ReleaseBundlePromotionFixture : IDisposable
    {
        ...
        public ReleaseCompatibilityMirrorRepairResult RepairCompatibilityMirrors()
        {
            var service = new ReleaseBundlePromotionService(
                ...
                new FixedTimeProvider(DateTimeOffset.Parse("2026-07-15T23:59:59Z")),
                PrivacyLaunchGate.ClearForTests);
            return service.RepairCompatibilityMirrors(CancellationToken.None);
        }
        ...
        private static string ArtifactRid(string platform, string arch)
            => platform.ToLowerInvariant() switch
            {
                "windows" => $"win-{arch}",
                "macos" => $"osx-{arch}",
                _ => $"{platform.ToLowerInvariant()}-{arch}"
            };
