...
- a valid close now requires:
  ...
  - `REPO_INVENTORY.yaml` covering the full mounted estate above with path, branch, HEAD SHA, and dirty-state fields
  - `ABSOLUTE_RELEASE_GATES.yaml`, `VERIFICATION_COMMANDS.md`, and `VERIFICATION_RESULTS.generated.json` covering every gate in `VERIFICATION_MATRIX.yaml`
  - `E2E_RESULTS.generated.json` covering the major user-journey gates, not only generic pass markers
...
3. keep the existing hub strict blockers honest
4. take the highest-value unresolved P0/P1 workpackage from `COMPLETION_WORKPACKAGES.yaml`
5. update canon first where needed, implement, verify, regenerate proof, update completion artifacts
6. repeat until the estate gate turns green
