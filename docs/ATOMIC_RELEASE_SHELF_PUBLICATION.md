...
14. Update top-level and cross-repository compatibility mirrors after commit. Mirror
    ...
    The Fleet-only
    `POST /api/internal/releases/repair-compatibility-mirrors` operation repairs the
    local top-level mirror from the active validated generation without changing
    `current.json`. Cross-repository legacy mirrors use
    `scripts/sync_active_release_compatibility_mirror.py`: run it without `--apply`
    first, then apply with a receipt path. It refuses authoritative targets and never
    reads release bytes from the mutable top-level compatibility mirror.
    If a rootless server cannot remove a stale, differently owned compatibility
    directory, move that exact entry to a
    `.release-shelf-compatibility-quarantine-*` sibling before retrying. The sibling is
    ignored by readers and source control, but remains a recoverable operator cleanup
    item until a host administrator removes it; never relax the writer policy or make
    the immutable generation writable.
