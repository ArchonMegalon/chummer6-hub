...
public static partial class PublicFacingCopyHumanizer
{
    ...
    [GeneratedRegex(@"\b(?:a|an)\s+review(?:\s+status)?\s+says\s+(?:confirmed\s+)?review(?:\s+details)?\s+(?:passed|ready)\s+on\s+the\s+return\s+path\.?", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
