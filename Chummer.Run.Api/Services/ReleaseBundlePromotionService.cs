...
public sealed record ReleaseCompatibilityMirrorRepairResult(
    string GenerationId,
...
public sealed class ReleaseBundlePromotionService
{
    ...
    /// <summary>
    /// Rebuilds the non-authoritative top-level compatibility mirrors from the
    /// currently active, fully validated immutable generation. This operation
    /// never rewrites current.json or any generation bytes.
    /// </summary>
    public ReleaseCompatibilityMirrorRepairResult RepairCompatibilityMirrors(
        CancellationToken cancellationToken)
    {
        string downloadsRoot = ResolveDownloadsRoot();
        EnsureDownloadsRootWritable(downloadsRoot);
        ...
        if (activeShelf.IsLegacy
            || string.IsNullOrWhiteSpace(activeShelf.GenerationId)
            || string.IsNullOrWhiteSpace(activeShelf.ReleaseVersion)
            || string.IsNullOrWhiteSpace(activeShelf.Channel)
            || string.IsNullOrWhiteSpace(activeShelf.InventoryDigest)
            || string.IsNullOrWhiteSpace(activeShelf.PointerDigest))
        {
            throw new InvalidOperationException(
                "compatibility mirror repair requires an activated layout-v1 generation.");
        }

        int mirroredFileCount = UpdateCompatibilityMirrorsStrict(
            activeShelf.PhysicalRoot,
            downloadsRoot,
            cancellationToken);

        // Use a fresh store so the final check cannot be satisfied by this
        // operation's initial in-memory snapshot.
        ReleaseShelfSnapshot verifiedShelf = new ReleaseShelfGenerationStore(_configuration).Capture();
        if (verifiedShelf.IsLegacy
            || !string.Equals(
                verifiedShelf.GenerationId,
                activeShelf.GenerationId,
                StringComparison.Ordinal)
            || !string.Equals(
                verifiedShelf.PointerDigest,
                activeShelf.PointerDigest,
                StringComparison.Ordinal)
            || !string.Equals(
                verifiedShelf.InventoryDigest,
                activeShelf.InventoryDigest,
                StringComparison.Ordinal))
        {
            throw new InvalidDataException(
                "active release shelf identity changed while compatibility mirrors were repaired.");
        }

        return new ReleaseCompatibilityMirrorRepairResult(
            activeShelf.GenerationId,
            activeShelf.ReleaseVersion,
            activeShelf.Channel,
            $"sha256:{activeShelf.InventoryDigest}",
        ...
        CancellationToken cancellationToken)
    {
        ...
        string liveCompatibilityManifestPath = Path.Combine(activeShelfRoot, CompatibilityManifestName);
        ...
        try
        {
            ...
            ValidatePreparedArtifactDeliveryContracts(stagedRoot);
            ...
            ReleaseBundlePromotionResult result = new(
                ...
                InstallDispatchUrls: promotedArtifactIds
                    .Select(id => publicShelfManifest.Downloads.First(artifact =>
    ...
    private PreparedReleaseBundle PrepareBundle(string bundleRoot)
    {
        ...
        JsonObject incomingCompatibilityManifestObject = LoadJsonObject(compatibilityManifestPath);
        ...
        CancellationToken cancellationToken)
    {
        ...
        JsonObject compatibilityObject = JsonSerializer.SerializeToNode(compatibilityManifest, JsonOptions)?.AsObject()
            ?? throw new InvalidOperationException("failed to materialize the compatibility release manifest.");
        Dictionary<string, GenerationArtifactRoute> artifactRoutes = compatibilityManifest.Downloads
            .Where(static artifact => !string.IsNullOrWhiteSpace(artifact.Id))
            .ToDictionary(
                static artifact => artifact.Id,
                static artifact => new GenerationArtifactRoute(
                    artifact.Id,
                    ResolveDownloadFileName(artifact),
                    string.IsNullOrWhiteSpace(artifact.PayloadFileName)
                        ? null
                        : Path.GetFileName(artifact.PayloadFileName.Trim()),
                    string.Equals(
                        artifact.InstallAccessClass?.Trim(),
                        "open_public",
                        StringComparison.OrdinalIgnoreCase)),
                StringComparer.OrdinalIgnoreCase);
        NormalizeManifestForGeneration(compatibilityObject, generationId, artifactRoutes);
        compatibilityObject["canonicalManifestUrl"] =
            $"/downloads/g/{generationId}/{CanonicalManifestName}";
        JsonObject normalizedCanonical = (JsonObject)canonicalManifest.DeepClone();
        NormalizeManifestForGeneration(normalizedCanonical, generationId, artifactRoutes);
        ...
        cancellationToken.ThrowIfCancellationRequested();
        WriteJsonFile(Path.Combine(stagedRoot, CompatibilityManifestName), compatibilityObject);
        cancellationToken.ThrowIfCancellationRequested();
        WriteJsonFile(Path.Combine(stagedRoot, CanonicalManifestName), normalizedCanonical);
        cancellationToken.ThrowIfCancellationRequested();
    }
        ...
        string generationId)
    {
        foreach (PublicReleaseArtifactDto artifact in compatibilityManifest.Downloads)
        {
            ...
            if (!string.IsNullOrWhiteSpace(artifact.PayloadAcquisitionMode))
            {
                normalizedSidecar["payloadAcquisitionMode"] =
                    NormalizePayloadAcquisitionMode(
                        artifact.PayloadAcquisitionMode,
                        artifactId);
            }
            WriteJsonFile(
    ...
    private static void ValidatePreparedArtifactDeliveryContracts(string stagedRoot)
    {
        ...
        IReadOnlyList<string> promotedArtifactIds)
    {
        ...
        JsonObject canonical = LoadJsonObject(Path.Combine(generationRoot, CanonicalManifestName));
        JsonObject compatibility = LoadJsonObject(Path.Combine(generationRoot, CompatibilityManifestName));
        if (!string.Equals(GetJsonString(canonical["generationId"]), pointer.GenerationId, StringComparison.Ordinal)
            || !string.Equals(GetJsonString(compatibility["generationId"]), pointer.GenerationId, StringComparison.Ordinal))
        {
            throw new InvalidDataException("release shelf manifests disagree about generationId.");
        }

        ValidateGenerationBoundManifestRoutes(canonical, pointer.GenerationId);
        ValidateGenerationBoundManifestRoutes(compatibility, pointer.GenerationId);
        if (!string.Equals(
                Sha256For(Path.Combine(generationRoot, CanonicalManifestName)),
    ...
    private void TryUpdateCompatibilityMirrors(string generationRoot, string downloadsRoot)
    {
        try
        {
            _ = UpdateCompatibilityMirrorsStrict(
                generationRoot,
                downloadsRoot,
    ...
    private static int UpdateCompatibilityMirrorsStrict(
        string generationRoot,
        string downloadsRoot,
        CancellationToken cancellationToken)
    {
        string[] directoryNames = ["files", "startup-smoke", "signing", "proof", "release-evidence"];
        string[] fileNames = [CanonicalManifestName, CompatibilityManifestName, "aur-packages.json"];
        string[] managedNames = [.. directoryNames, .. fileNames];
        string transactionRoot = Path.Combine(
            downloadsRoot,
            $".release-shelf-mirror-repair-{Guid.NewGuid():N}");
        string stagedRoot = Path.Combine(transactionRoot, "staged");
        string backupRoot = Path.Combine(transactionRoot, "backup");
        var backedUpNames = new List<string>();
        ...
        try
        {
            ...
            foreach (string directoryName in directoryNames)
            {
                ...
                {
                    EnsureRegularDirectory(source, $"release generation {directoryName} directory");
                    CopyDirectoryContents(
                        source,
                        Path.Combine(stagedRoot, directoryName),
                        cancellationToken);
                }
            }
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                string source = Path.Combine(generationRoot, fileName);
                if (!File.Exists(source))
                {
                    if (fileName is CanonicalManifestName or CompatibilityManifestName)
                    {
                        throw new InvalidDataException(
                            $"active release generation is missing required manifest {fileName}.");
                    }

                    continue;
                }

                EnsureRegularFile(source, $"release generation {fileName}");
                string staged = Path.Combine(stagedRoot, fileName);
                File.Copy(source, staged);
                MakeCompatibilityMirrorFileWritable(staged);
            }

            IReadOnlyDictionary<string, string> sourceSnapshot =
                SnapshotCompatibilityMirrorFiles(generationRoot, managedNames);
            AssertCompatibilityMirrorSnapshot(
                sourceSnapshot,
                SnapshotCompatibilityMirrorFiles(stagedRoot, managedNames));

            var existingEntries = new Dictionary<string, string>(StringComparer.Ordinal);
            foreach (string managedName in managedNames)
            {
                string? existing = FindCanonicalCompatibilityEntry(downloadsRoot, managedName);
                if (existing is null)
                {
                    continue;
                }
                ...
                {
                    throw new InvalidDataException(
                        $"compatibility mirror entry {managedName} cannot be a link.");
                }

                bool shouldBeDirectory = directoryNames.Contains(managedName, StringComparer.Ordinal);
                bool isDirectory = (attributes & FileAttributes.Directory) != 0;
                if (shouldBeDirectory != isDirectory)
                {
                    throw new InvalidDataException(
                        $"compatibility mirror entry {managedName} has the wrong filesystem type.");
                }

                existingEntries.Add(managedName, existing);
            }

            foreach ((string managedName, string existing) in existingEntries)
            {
                cancellationToken.ThrowIfCancellationRequested();
                MoveCompatibilityEntry(existing, Path.Combine(backupRoot, managedName));
                backedUpNames.Add(managedName);
            }

            foreach (string managedName in managedNames)
            {
                cancellationToken.ThrowIfCancellationRequested();
                string staged = Path.Combine(stagedRoot, managedName);
                if (File.Exists(staged) || Directory.Exists(staged))
                {
                    MoveCompatibilityEntry(staged, Path.Combine(downloadsRoot, managedName));
                    installedNames.Add(managedName);
                }
            }

            AssertCompatibilityMirrorSnapshot(
                sourceSnapshot,
                SnapshotCompatibilityMirrorFiles(downloadsRoot, managedNames));
            FlushDirectoryDurably(downloadsRoot);
            committed = true;
            return sourceSnapshot.Count;
        }
        catch
        {
            if (!committed)
            {
                ...
                try
                {
                    foreach (string managedName in installedNames)
                    {
                        string installed = Path.Combine(downloadsRoot, managedName);
                        DeleteCompatibilityEntryIfPresent(installed);
                    }

                    foreach (string managedName in backedUpNames)
                    {
                        string backup = Path.Combine(backupRoot, managedName);
                        MoveCompatibilityEntry(
                            backup,
                            Path.Combine(downloadsRoot, managedName));
                    }
                ...
                if (rollbackFailure is not null)
                {
                    throw new IOException(
                        "compatibility mirror repair failed and its prior mirrors could not be fully restored.",
                        rollbackFailure);
                }
            }
        ...
        finally
        {
            ...
            {
                // The verified mirrors are already committed; a private staging
                // directory cleanup failure must not report the repair as failed.
            }
        }
    }

    private static IReadOnlyDictionary<string, string> SnapshotCompatibilityMirrorFiles(
        string root,
        IReadOnlyCollection<string> managedNames)
    {
        ...
        foreach (string managedName in managedNames)
        {
            string path = Path.Combine(root, managedName);
            if (Directory.Exists(path))
            {
                EnsureRegularDirectory(path, $"compatibility mirror {managedName} directory");
                foreach (string file in EnumerateRegularFilesWithoutLinks(path))
                {
                    ...
                    snapshot.Add(
                        relative,
                        $"{input.Length}:{Convert.ToHexStringLower(SHA256.HashData(input))}");
                }
            }
            else if (File.Exists(path))
            {
                EnsureRegularFile(path, $"compatibility mirror {managedName}");
    ...
    private static void AssertCompatibilityMirrorSnapshot(
        ...
        IReadOnlyDictionary<string, string> actual)
    {
        ...
        {
            throw new InvalidDataException(
                "compatibility mirror bytes do not match the active immutable generation.");
        }
    }

    private static string? FindCanonicalCompatibilityEntry(string root, string name)
    {
        string[] matches = Directory.EnumerateFileSystemEntries(root)
        ...
        {
            throw new InvalidDataException(
                $"compatibility mirror entry {name} has ambiguous or noncanonical casing.");
        }

        return matches[0];
    }

    private static void MoveCompatibilityEntry(string source, string destination)
    {
        FileAttributes attributes = File.GetAttributes(source);
        if ((attributes & FileAttributes.ReparsePoint) != 0)
        {
            throw new InvalidDataException("compatibility mirror transaction cannot move links.");
        }
    ...
    private static void DeleteCompatibilityEntryIfPresent(string path)
    {
        ...
        if ((attributes & FileAttributes.ReparsePoint) != 0)
        {
            throw new InvalidDataException("compatibility mirror transaction cannot delete links.");
        }
        ...
        DateTimeOffset evaluatedAtUtc)
    {
        ...
        {
            bool allowWindowsCompatibilityPreview =
                AllowsWindowsCompatibilityPreview(
                    artifact,
                    promotionEvidence,
                    compatibilityManifest);
        ...
        bool requireIncomingUrls = true)
    {
        ...
        (string? payloadFileName, string? payloadUrl, string? payloadSha256, long? payloadSizeBytes) =
            NormalizePayloadContract(
                ...
                "canonical",
                requireIncomingUrls);
        string? payloadAcquisitionMode = payloadFileName is null
            ? null
            : NormalizeOptionalPayloadAcquisitionMode(
                artifact.PayloadAcquisitionMode,
                artifactId);
        ...
        bool requireIncomingUrls = true)
    {
        ...
        return new NormalizedArtifactContract(
            ...
            RequireArtifactLabel(
                string.IsNullOrWhiteSpace(artifact.PlatformLabel)
                    ? artifact.Platform
        ...
        string artifactId)
    {
        ...
        string canonicalExpected = $"https://chummer.run{expected}";
        if (url.Contains('?', StringComparison.Ordinal)
            || url.Contains('#', StringComparison.Ordinal)
            || url.Contains('\\', StringComparison.Ordinal)
            || !(string.Equals(url, expected, StringComparison.Ordinal)
                 || string.Equals(url, canonicalExpected, StringComparison.Ordinal)))
        {
            throw new InvalidDataException(
                $"artifact {artifactId} URL must be the governed incoming path {expected} "
                + $"or its exact canonical public URL {canonicalExpected}.");
        }

        // Registry-authored public receipts use the canonical absolute URL,
        // while local bundle builders use the relative incoming route. Bind
        // both spellings to one internal value before comparing manifests or
        // rewriting them to immutable generation routes.
        return expected;
    }
    ...
    private static string? NormalizeOptionalPayloadAcquisitionMode(
        string? value,
        string artifactId)
        => string.IsNullOrWhiteSpace(value)
            ? null
            : NormalizePayloadAcquisitionMode(value, artifactId);

    private static string NormalizePayloadAcquisitionMode(
        string? value,
        string artifactId)
    {
        string mode = NormalizeOptionalToken(value) ?? string.Empty;
        if (mode is not ("download" or "embedded"))
        {
            throw new InvalidDataException(
                $"artifact {artifactId} payloadAcquisitionMode must be download or embedded.");
        }
        ...
        bool allowWindowsCompatibilityPreview)
    {
        ...
        {
            if (allowWindowsCompatibilityPreview)
            {
                ValidateWindowsCompatibilityStartupSmoke(receipt, artifact.ArtifactId);
            }
            else
            {
    ...
    private static void ValidateWindowsCompatibilityStartupSmoke(
        StartupSmokeReceipt receipt,
        string artifactId)
    {
        NativeWindowsHostEvidence? evidence = receipt.NativeHostEvidence;
        if (NormalizeToken(receipt.ExecutionEnvironment) != "wine_compatibility"
            || NormalizeToken(receipt.VerificationScope) != "windows_compatibility_startup"
            || evidence is null
            || !string.Equals(
                evidence.ContractName?.Trim(),
                "chummer6-ui.native_windows_host_evidence",
                StringComparison.Ordinal)
            || NormalizeToken(evidence.Status) != "not_native"
            || evidence.IsNativeWindows is not false
            || NormalizePlatform(evidence.HostPlatform) != "linux"
            || NormalizeToken(evidence.HostKernel) != "linux"
            || !NormalizeToken(evidence.Runner).Contains("wine", StringComparison.Ordinal)
            || NormalizeToken(evidence.EvidenceSource) != "wine_runner_selection")
        {
            throw new InvalidDataException(
                $"preview Windows compatibility startup evidence is not fail-closed for {artifactId}.");
        }
    }

    private static bool AllowsWindowsCompatibilityPreview(
        CanonicalArtifactRecord artifact,
        PromotionEvidenceDocument evidence,
        PublicReleaseManifestDto manifest)
    {
        if (NormalizePlatform(artifact.Platform) != "windows"
            || NormalizeToken(manifest.Channel) != "preview"
            || NormalizeToken(manifest.Status) != "published"
            || NormalizeToken(manifest.RolloutState) != "coverage_incomplete"
            || NormalizeToken(manifest.SupportabilityState) != "review_required")
        {
            return false;
        }

        PromotionArtifactEvidence? artifactEvidence = FindPromotionEvidence(
            artifact,
            evidence);
        return artifactEvidence is not null
               && artifactEvidence.NativeWindowsStartupProofRequired is false
               && NormalizeToken(artifactEvidence.StartupSmokeExecutionEnvironment)
                   == "wine_compatibility"
               && NormalizeToken(artifactEvidence.StartupSmokeStatus) == "pass"
               && NormalizeToken(artifactEvidence.SigningStatus) == "skipped_preview";
    }

    private static PromotionArtifactEvidence? FindPromotionEvidence(
        CanonicalArtifactRecord artifact,
        PromotionEvidenceDocument evidence)
        => evidence.Artifacts.FirstOrDefault(item =>
            string.Equals(item.ArtifactId, artifact.ArtifactId, StringComparison.OrdinalIgnoreCase)
            || string.Equals(
                item.FileName,
                ResolveArtifactFileName(artifact.FileName, artifact.DownloadUrl),
                StringComparison.OrdinalIgnoreCase));
        ...
        IReadOnlyList<string> promotedArtifactIds)
    {
        ...
        return liveCompatibilityManifest;
    }

    private static void ValidatePrivacyReadinessCoherence(
        PublicReleaseManifestDto compatibilityManifest,
        JsonObject canonicalManifest)
    {
    ...
    private sealed record CanonicalArtifactRecord(
        ...
        [property: JsonPropertyName("payloadAcquisitionMode")] string? PayloadAcquisitionMode);
    ...
    private sealed record StartupSmokeReceipt(
        ...
        [property: JsonPropertyName("verificationScope")] string? VerificationScope,
    ...
    private sealed record PromotionArtifactEvidence(
        ...
        [property: JsonPropertyName("startupSmokeExecutionEnvironment")] string? StartupSmokeExecutionEnvironment,
        [property: JsonPropertyName("nativeWindowsStartupProofRequired")] bool? NativeWindowsStartupProofRequired,
