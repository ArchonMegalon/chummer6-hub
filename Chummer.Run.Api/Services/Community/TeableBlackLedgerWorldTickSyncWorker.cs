...
public sealed class TeableBlackLedgerWorldTickSyncWorker : BackgroundService
{
    private static readonly TimeSpan DefaultInitialDelay = TimeSpan.FromSeconds(45);
    ...
    private readonly ILogger<TeableBlackLedgerWorldTickSyncWorker> _logger;

    public TeableBlackLedgerWorldTickSyncWorker(
        TeableBlackLedgerWorldTickService teableWorldTicks,
        IConfiguration configuration,
        ILogger<TeableBlackLedgerWorldTickSyncWorker> logger)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        TimeSpan initialDelay = ResolveDurationSeconds("CHUMMER_TEABLE_BLACK_LEDGER_RECONCILE_INITIAL_DELAY_SECONDS", DefaultInitialDelay);
        TimeSpan interval = ResolveDurationMinutes("CHUMMER_TEABLE_BLACK_LEDGER_RECONCILE_INTERVAL_MINUTES", DefaultInterval);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                TeableBlackLedgerWorldTickSyncResult result = await _teableWorldTicks.SyncAllAsync(stoppingToken);
                if (string.Equals(result.State, "passed", StringComparison.OrdinalIgnoreCase))
                {
                    _logger.LogInformation(
                        "Teable BLACK LEDGER world-tick reconciliation synced {SyncedCount}/{AttemptedCount} rows into {TableId}.",
                        result.SyncedCount,
                ...
                {
                    _logger.LogWarning(
                        "Teable BLACK LEDGER world-tick reconciliation ended in {State} with {FailedCount} failed rows: {Errors}",
                        result.State,
                        result.FailedCount,
            ...
            {
                _logger.LogWarning(ex, "Teable BLACK LEDGER world-tick reconciliation loop failed.");
            }
