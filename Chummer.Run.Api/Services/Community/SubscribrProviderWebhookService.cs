...
public sealed class SubscribrProviderWebhookService
{
    ...
    {
        string material = SignatureMaterial(CanonicalPayload(request), timestamp);
        ...
        DateTimeOffset? now = null)
    {
        ...
        if (!VerifySignature(canonicalPayload, signature, timestamp))
        {
        ...
        lock (_store.Gate)
        {
            ...
            string packetPath = Clean(request.PacketPath);
            string markdownExportPath = Clean(request.MarkdownExportPath);
            if (!File.Exists(packetPath) || !File.Exists(markdownExportPath))
            {
                return RecordRejectedLocked(eventId, eventType, "verified", "first_seen", "blocked", "packet and markdown export must exist", processedAtUtc);
            }

            JsonObject packet = LoadPacket(packetPath);
            ...
            string receiptPath = Path.Combine(_store.ReceiptRoot, $"{eventId}.generated.json");
            string packetId = packet["packet_id"]?.GetValue<string>() ?? string.Empty;
            string mode = packet["mode"]?.GetValue<string>() ?? string.Empty;
            ...
            var receipt = new JsonObject
            {
                ...
                ["source_packet_sha256"] = Sha256File(packetPath),
                ["script_markdown_sha256"] = Sha256File(markdownExportPath),
