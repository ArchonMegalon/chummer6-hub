...
public sealed class RegistryTruthBindingService
{
    ...
    private static RegistryTruthBindingProjection BuildDownloadsBinding(
        ...
        string locale)
    {
        string summary = $"Downloads stays bound to {manifest.Channel} {manifest.Version}, so the installer, status, and support copy cannot drift apart.";
        return new RegistryTruthBindingProjection(
            ...
            [
                artifact is null
                    ? "Downloads has no current installer yet, so the page must stay quiet about install status."
                    : $"{artifact.Id} is the current installer on Downloads.",
                $"{ResolveProofStatus(manifest)} release status and {ResolveSupportabilityState(manifest)} support state come from Chummer.",
                ResolveFixAvailabilitySummary(manifest)
            ],
            Actions:
            [
                new RegistryTruthBindingActionProjection("open_downloads", "Open downloads", "/downloads", "Inspect the current installer."),
                new RegistryTruthBindingActionProjection("open_current_release", "Open current release", "/now", "Compare the same current release status on /now."),
                new RegistryTruthBindingActionProjection("open_status", "Open status", "/status", "Check the same status and cautions without inventing a second release story.")
            ],
            EmittedAtUtc: now,
            Locale: locale,
    ...
    private static RegistryTruthBindingProjection BuildInstallHelpBinding(
        ...
        string locale)
    {
        string summary = "Known issues and install help stay on Downloads, while every caution, fix note, and support line comes from the release registry.";
        return new RegistryTruthBindingProjection(
            ...
            [
                ...
                $"Current support status is {ResolveSupportabilityState(manifest)}.",
                ResolveFixAvailabilitySummary(manifest)
            ],
            Actions:
            [
                new RegistryTruthBindingActionProjection("open_downloads_help", "Open install help", "/downloads", "Keep install questions on Downloads where current cautions already live."),
                new RegistryTruthBindingActionProjection("open_status", "Open status", "/status", "Open Status when install help needs more than the page summary."),
        ...
        string locale)
    {
        ...
        string summary = installation is not null
            ? $"Account-aware guidance keeps claimed install {installation.Platform ?? "desktop"} {installation.Version} on registry-backed release status without changing the published bytes."
            : $"Record {receipt!.ReceiptId} keeps the public download path open while account return stays attached to this Chummer install.";
        ...
        return new RegistryTruthBindingProjection(
            ...
            EvidenceLines:
            [
                "The published packages stay the same for every user; only the account claim step is account-aware.",
                installation is null
                    ? $"Download record {receipt!.ReceiptId} stays attached to this Chummer install."
                    : $"Claimed installation {installation.InstallationId} keeps continuity on /account/access.",
                $"{ResolveProofStatus(manifest)} release status and {ResolveSupportabilityState(manifest)} support state still come from Chummer."
            ],
            Actions:
            [
                new RegistryTruthBindingActionProjection("open_account_access", "Open Devices & access", "/account/access", "Review signed-in account return."),
        ...
        string locale)
    {
        ...
        if (supportCase is not null)
        {
            ...
            string summary = $"{concierge.SupportClosure.Summary} Support recovery stays tied to registry-backed release status before any recovery step is suggested.";
            return new RegistryTruthBindingProjection(
                ...
                [
                    ...
                    artifact is null
                        ? "Support recovery has no current installer route yet, so recovery must stay on Chummer support surfaces."
                        : $"{artifact.Id} is the installer named by the same recovery path."
                ],
                Actions:
                [
        ...
        string genericSummary = "Support recovery stays in Chummer and stays attached to the linked copy; when release status is missing, the recovery path must say so instead of inventing a detached workaround.";
        return new RegistryTruthBindingProjection(
            ...
            EvidenceLines:
            [
                "Support recovery remains attached to the linked copy.",
                $"{ResolveProofStatus(manifest)} release status and {ResolveSupportabilityState(manifest)} support state still come from Chummer.",
                ResolveFixAvailabilitySummary(manifest)
            ],
            Actions:
    ...
    private static RegistryTruthBindingProjection BuildPublicReleaseShelfBinding(
        ...
        string locale)
    {
        string summary = $"The public pages keep /now, /status, and /downloads on the same {manifest.Channel} {manifest.Version} release.";
        return new RegistryTruthBindingProjection(
            ...
            [
                artifact is null
                    ? "The current release does not yet name a promoted installer."
                    : $"{artifact.Id} is the installer shared by the public release and Downloads.",
                $"Public release status is {ResolveProofStatus(manifest)}.",
                ResolveFixAvailabilitySummary(manifest)
            ],
            Actions:
            [
                new RegistryTruthBindingActionProjection("open_current_release", "Open current release", "/now", "Inspect the current public release."),
                new RegistryTruthBindingActionProjection("open_status", "Open status", "/status", "Compare status and cautions on the mirrored status route."),
                new RegistryTruthBindingActionProjection("open_downloads", "Open downloads", "/downloads", "See the same release status on Downloads.")
            ],
            EmittedAtUtc: now,
            Locale: locale,
            SourceId: artifact?.Id ?? manifest.Version);
    ...
    private static string ResolveFixAvailabilitySummary(PublicReleaseManifestDto manifest)
        => string.IsNullOrWhiteSpace(manifest.FixAvailabilitySummary)
            ? "The release registry has not published a fix-availability summary yet."
