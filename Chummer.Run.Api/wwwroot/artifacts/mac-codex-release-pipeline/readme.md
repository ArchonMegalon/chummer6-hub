...
7. generates both `releases.json` and `RELEASE_CHANNEL.generated.json` for the combined bundle
8. writes `release-evidence/public-promotion.json`
9. uploads every governed file through a durable session rooted at `https://chummer.run/api/internal/releases/upload-sessions`, then completes that same session exactly once
...
10. verifies the promoted public shelf and prints the live `/downloads/install/{artifactId}` handoff URLs
11. records signed-in claim-code handoffs in the upload response and redacts them from stdout unless you explicitly opt in to print them
12. logs the executing bootstrap source path and SHA-256 so drift is visible in the transcript
...
7. Upload auth goes through a `0600` temp curl config file without putting the token in `curl` argv. The cleanup trap is installed before that file can be created and removes it on success or failure.
