...
public sealed class InternalReleaseBundlesController : ControllerBase
{
    ...
    [HttpPost("/api/internal/releases/repair-compatibility-mirrors")]
    [IgnoreAntiforgeryToken]
    [ProducesResponseType<ReleaseCompatibilityMirrorRepairResult>(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status403Forbidden)]
    public ActionResult<ReleaseCompatibilityMirrorRepairResult> RepairCompatibilityMirrors(
        CancellationToken cancellationToken)
    {
        ...
        if (authorization!.UploadTicketClaims is not null)
        {
            return BuildProblem(
                ...
                "compatibility mirror repair requires the Fleet internal authorization boundary.",
                "https://chummer.run/problems/release-bundle/fleet-auth-required");
        }

        try
        {
            return Ok(_promotionService.RepairCompatibilityMirrors(cancellationToken));
        }
        catch (Exception ex) when (ex is InvalidDataException or InvalidOperationException)
        {
            return BuildProblem(
                StatusCodes.Status409Conflict,
                "Compatibility mirror repair rejected",
                ex.Message,
                "https://chummer.run/problems/release-bundle/compatibility-mirror-repair-rejected");
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            _logger.LogError(ex, "Release compatibility mirror repair failed.");
            return BuildProblem(
                StatusCodes.Status503ServiceUnavailable,
                "Compatibility mirror repair unavailable",
                "release compatibility mirror storage is unavailable.",
