...
async function assertPublicArtifactDetail(page, pageErrors, path, label) {
  await gotoAndAssert(page, pageErrors, path, async () => {
    const currentPath = new URL(page.url()).pathname;
...
function resolveInstallNextFromHref(href, label) {
  assert.equal(Boolean(href), true, `${label} should expose a non-empty href.`);
  ...
  if (href.startsWith('/downloads/install/')) {
    return href;
  }
...
function assertGoogleOauthUrl(url, label) {
  assert.equal(url.hostname, 'accounts.google.com', `${label} should hand off to Google OAuth.`);
  assert(
    url.pathname === '/o/oauth2/v2/auth' || url.pathname === '/v3/signin/identifier',
    `${label} should land on a Google OAuth/sign-in step, got ${url.pathname}.`
  );
  assert.equal(
    url.searchParams.get('redirect_uri'),
    'https://chummer.run/auth/google/callback',
    `${label} should keep the Chummer Google callback.`
  );
  assert(url.searchParams.has('state'), `${label} should preserve OAuth state.`);
}

async function assertGoogleOauthRedirect(page, label) {
  const current = new URL(page.url());
  if (current.hostname === 'accounts.google.com') {
    assertGoogleOauthUrl(current, label);
    return;
  }

  assert.equal(current.hostname, 'chummer.run', `${label} should stay on the local Chummer host only as a reverse-proxy handoff.`);
  let handoffUrl = current;
  if (current.pathname === '/login') {
    const googleHref = await page.locator('a[href^="/auth/google/start"]').first().getAttribute('href');
    assert(googleHref, `${label} login page should expose a Google auth handoff link.`);
    handoffUrl = new URL(googleHref, current.origin);
  } else {
    assert.equal(current.pathname, '/auth/google/start', `${label} should stop only on the local Google start route before provider handoff.`);
  }
  const response = await page.request.get(handoffUrl.toString(), { maxRedirects: 0 });
  assert.equal(response.status(), 302, `${label} local Google start should issue an OAuth redirect.`);
  const location = response.headers().location;
  assert(location, `${label} local Google start should return a Location header.`);
  assertGoogleOauthUrl(new URL(location), label);
}
...
(async () => {
  ...
  const codexRedirectResponse = await page.context().request.get(`${baseUrl}/participate/codex`, {
  ...
  await assertPublicArtifactDetail(page, pageErrors, publicArtifactDetailPath, '/artifacts -> public artifact detail');
  ...
  if (isLocalReverseProxyMode) {
    ...
    await browser.close();
    console.log('hub-playwright: completed local reverse-proxy flow');
    return;
  }

  assert(page.url().includes(signupNext), 'Signup callback should land on the signed-in handoff route.');
  await expectVisible(page, 'text=Claim code');
  await assertNoPageErrors(page, pageErrors, 'Download handoff');

  const installSlug = signupNext.split('/').pop();
  assert.equal(Boolean(installSlug), true, `Unable to derive install slug from ${signupNext}.`);
  const downloadRequest = page.waitForResponse((response) => {
    const url = response.url();
    return url.includes(`/downloads/file/${installSlug}`) && response.status() === 200;
  });
  ...
  await expectBodyText(page, 'Generate aftermath or replay package', '/account/work/workspaces detail');
  ...
  await page.fill('#prepQuery', 'debriefed');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=debriefed/.test(page.url()), 'Workspace detail search should preserve the debriefed continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail debriefed search');
  await expectBodyText(page, 'match(es) for "debriefed"', '/account/work/workspaces detail debriefed search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail debriefed search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail debriefed search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail debriefed search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail debriefed search');
  const workspaceDebriefedSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceDebriefedSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the debriefed continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail debriefed search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail debriefed search');
  ...
  await page.fill('#prepQuery', 'outbriefed');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=outbriefed/.test(page.url()), 'Workspace detail search should preserve the outbriefed continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail outbriefed search');
  await expectBodyText(page, 'match(es) for "outbriefed"', '/account/work/workspaces detail outbriefed search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail outbriefed search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail outbriefed search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail outbriefed search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail outbriefed search');
  const workspaceOutbriefedSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceOutbriefedSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the outbriefed continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail outbriefed search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail outbriefed search');

  await page.fill('#prepQuery', 'outbriefing');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=outbriefing/.test(page.url()), 'Workspace detail search should preserve the outbriefing continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail outbriefing search');
  await expectBodyText(page, 'match(es) for "outbriefing"', '/account/work/workspaces detail outbriefing search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail outbriefing search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail outbriefing search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail outbriefing search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail outbriefing search');
  const workspaceOutbriefingSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceOutbriefingSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the outbriefing continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail outbriefing search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail outbriefing search');

  await page.fill('#prepQuery', 'outbriefings');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=outbriefings/.test(page.url()), 'Workspace detail search should preserve the outbriefings continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail outbriefings search');
  await expectBodyText(page, 'match(es) for "outbriefings"', '/account/work/workspaces detail outbriefings search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail outbriefings search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail outbriefings search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail outbriefings search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail outbriefings search');
  const workspaceOutbriefingsSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceOutbriefingsSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the outbriefings continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail outbriefings search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail outbriefings search');
  ...
  await page.fill('#prepQuery', 'out briefed');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=out(?:%20|\+)briefed/.test(page.url()), 'Workspace detail search should preserve the split out briefed continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail out briefed split search');
  await expectBodyText(page, 'match(es) for "out briefed"', '/account/work/workspaces detail out briefed split search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail out briefed split search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail out briefed split search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail out briefed split search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail out briefed split search');
  const workspaceOutBriefedSplitSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceOutBriefedSplitSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the split out briefed continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail out briefed split search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail out briefed split search');

  await page.fill('#prepQuery', 'out briefing');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=out(?:%20|\+)briefing/.test(page.url()), 'Workspace detail search should preserve the split out briefing continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail out briefing split search');
  await expectBodyText(page, 'match(es) for "out briefing"', '/account/work/workspaces detail out briefing split search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail out briefing split search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail out briefing split search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail out briefing split search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail out briefing split search');
  const workspaceOutBriefingSplitSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceOutBriefingSplitSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the split out briefing continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail out briefing split search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail out briefing split search');

  await page.fill('#prepQuery', 'out briefings');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=out(?:%20|\+)briefings/.test(page.url()), 'Workspace detail search should preserve the split out briefings continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail out briefings split search');
  await expectBodyText(page, 'match(es) for "out briefings"', '/account/work/workspaces detail out briefings split search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail out briefings split search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail out briefings split search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail out briefings split search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail out briefings split search');
  const workspaceOutBriefingsSplitSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceOutBriefingsSplitSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the split out briefings continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail out briefings split search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail out briefings split search');
  ...
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=out-briefed/.test(page.url()), 'Workspace detail search should preserve the hyphen out-briefed continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail out-briefed hyphen search');
  await expectBodyText(page, 'match(es) for "out-briefed"', '/account/work/workspaces detail out-briefed hyphen search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail out-briefed hyphen search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail out-briefed hyphen search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail out-briefed hyphen search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail out-briefed hyphen search');
  const workspaceOutBriefedHyphenSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceOutBriefedHyphenSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the hyphen out-briefed continuity prep query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail out-briefed hyphen search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail out-briefed hyphen search');
  ...
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=out-briefing/.test(page.url()), 'Workspace detail search should preserve the hyphen out-briefing continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail out-briefing hyphen search');
  await expectBodyText(page, 'match(es) for "out-briefing"', '/account/work/workspaces detail out-briefing hyphen search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail out-briefing hyphen search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail out-briefing hyphen search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail out-briefing hyphen search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail out-briefing hyphen search');
  const workspaceOutBriefingHyphenSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceOutBriefingHyphenSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the hyphen out-briefing continuity prep query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail out-briefing hyphen search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail out-briefing hyphen search');
  ...
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=out-briefings/.test(page.url()), 'Workspace detail search should preserve the hyphen out-briefings continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail out-briefings hyphen search');
  await expectBodyText(page, 'match(es) for "out-briefings"', '/account/work/workspaces detail out-briefings hyphen search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail out-briefings hyphen search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail out-briefings hyphen search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail out-briefings hyphen search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail out-briefings hyphen search');
  const workspaceOutBriefingsHyphenSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceOutBriefingsHyphenSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the hyphen out-briefings continuity prep query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail out-briefings hyphen search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail out-briefings hyphen search');
  ...
  await page.fill('#prepQuery', 'postmortems');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=postmortems/.test(page.url()), 'Workspace detail search should preserve the compact postmortems continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail postmortems compact search');
  await expectBodyText(page, 'match(es) for "postmortems"', '/account/work/workspaces detail postmortems compact search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail postmortems compact search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail postmortems compact search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail postmortems compact search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail postmortems compact search');
  const workspacePostmortemsCompactSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostmortemsCompactSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the compact postmortems continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail postmortems compact search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail postmortems compact search');

  await page.fill('#prepQuery', 'post mortems');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=post(?:%20|\+)mortems/.test(page.url()), 'Workspace detail search should preserve the split post mortems continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail post mortems split search');
  await expectBodyText(page, 'match(es) for "post mortems"', '/account/work/workspaces detail post mortems split search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail post mortems split search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail post mortems split search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail post mortems split search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail post mortems split search');
  const workspacePostMortemsSplitSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostMortemsSplitSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the split post mortems continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail post mortems split search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail post mortems split search');
  ...
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=post-mortems/.test(page.url()), 'Workspace detail search should preserve the hyphen post-mortems continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail post-mortems hyphen search');
  await expectBodyText(page, 'match(es) for "post-mortems"', '/account/work/workspaces detail post-mortems hyphen search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail post-mortems hyphen search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail post-mortems hyphen search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail post-mortems hyphen search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail post-mortems hyphen search');
  const workspacePostMortemsHyphenSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostMortemsHyphenSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the hyphen post-mortems continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail post-mortems hyphen search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail post-mortems hyphen search');
  ...
  await page.fill('#prepQuery', 'postsessions');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=postsessions/.test(page.url()), 'Workspace detail search should preserve the compact postsessions continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail postsessions compact search');
  await expectBodyText(page, 'match(es) for "postsessions"', '/account/work/workspaces detail postsessions compact search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail postsessions compact search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail postsessions compact search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail postsessions compact search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail postsessions compact search');
  const workspacePostsessionsCompactSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostsessionsCompactSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the compact postsessions continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail postsessions compact search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail postsessions compact search');

  await page.fill('#prepQuery', 'post sessions');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=post(?:%20|\+)sessions/.test(page.url()), 'Workspace detail search should preserve the split post sessions continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail post sessions split search');
  await expectBodyText(page, 'match(es) for "post sessions"', '/account/work/workspaces detail post sessions split search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail post sessions split search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail post sessions split search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail post sessions split search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail post sessions split search');
  const workspacePostSessionsSplitSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostSessionsSplitSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the split post sessions continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail post sessions split search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail post sessions split search');
  ...
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=post-sessions/.test(page.url()), 'Workspace detail search should preserve the hyphen post-sessions continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail post-sessions hyphen search');
  await expectBodyText(page, 'match(es) for "post-sessions"', '/account/work/workspaces detail post-sessions hyphen search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail post-sessions hyphen search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail post-sessions hyphen search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail post-sessions hyphen search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail post-sessions hyphen search');
  const workspacePostSessionsHyphenSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostSessionsHyphenSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the hyphen post-sessions continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail post-sessions hyphen search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail post-sessions hyphen search');
  ...
  await page.fill('#prepQuery', 'postruns');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=postruns/.test(page.url()), 'Workspace detail search should preserve the compact postruns continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail postruns compact search');
  await expectBodyText(page, 'match(es) for "postruns"', '/account/work/workspaces detail postruns compact search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail postruns compact search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail postruns compact search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail postruns compact search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail postruns compact search');
  const workspacePostrunsCompactSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostrunsCompactSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the compact postruns continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail postruns compact search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail postruns compact search');

  await page.fill('#prepQuery', 'post runs');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=post(?:%20|\+)runs/.test(page.url()), 'Workspace detail search should preserve the split post runs continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail post runs split search');
  await expectBodyText(page, 'match(es) for "post runs"', '/account/work/workspaces detail post runs split search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail post runs split search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail post runs split search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail post runs split search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail post runs split search');
  const workspacePostRunsSplitSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostRunsSplitSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the split post runs continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail post runs split search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail post runs split search');
  ...
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=post-runs/.test(page.url()), 'Workspace detail search should preserve the hyphen post-runs continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail post-runs hyphen search');
  await expectBodyText(page, 'match(es) for "post-runs"', '/account/work/workspaces detail post-runs hyphen search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail post-runs hyphen search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail post-runs hyphen search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail post-runs hyphen search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail post-runs hyphen search');
  const workspacePostRunsHyphenSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostRunsHyphenSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the hyphen post-runs continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail post-runs hyphen search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail post-runs hyphen search');
  ...
  await page.fill('#prepQuery', 'postgames');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=postgames/.test(page.url()), 'Workspace detail search should preserve the compact postgames continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail postgames compact search');
  await expectBodyText(page, 'match(es) for "postgames"', '/account/work/workspaces detail postgames compact search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail postgames compact search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail postgames compact search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail postgames compact search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail postgames compact search');
  const workspacePostgamesCompactSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostgamesCompactSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the compact postgames continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail postgames compact search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail postgames compact search');
  ...
  await page.fill('#prepQuery', 'post games');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=post(?:%20|\+)games/.test(page.url()), 'Workspace detail search should preserve the split post games continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail post games split search');
  await expectBodyText(page, 'match(es) for "post games"', '/account/work/workspaces detail post games split search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail post games split search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail post games split search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail post games split search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail post games split search');
  const workspacePostGamesSplitSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostGamesSplitSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the split post games continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail post games split search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail post games split search');
  ...
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=post-games/.test(page.url()), 'Workspace detail search should preserve the hyphen post-games continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail post-games hyphen search');
  await expectBodyText(page, 'match(es) for "post-games"', '/account/work/workspaces detail post-games hyphen search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail post-games hyphen search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail post-games hyphen search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail post-games hyphen search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail post-games hyphen search');
  const workspacePostGamesHyphenSearchText = await page.locator('body').innerText();
  assert.equal(
    workspacePostGamesHyphenSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the hyphen post-games continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail post-games hyphen search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail post-games hyphen search');
  ...
  await page.fill('#prepQuery', 'lessonlearned');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=lessonlearned/.test(page.url()), 'Workspace detail search should preserve the compact lessonlearned continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail lessonlearned compact search');
  await expectBodyText(page, 'match(es) for "lessonlearned"', '/account/work/workspaces detail lessonlearned compact search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail lessonlearned compact search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail lessonlearned compact search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail lessonlearned compact search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail lessonlearned compact search');
  const workspaceLessonLearnedCompactSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceLessonLearnedCompactSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the compact lessonlearned continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail lessonlearned compact search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail lessonlearned compact search');

  await page.fill('#prepQuery', 'lessonslearned');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=lessonslearned/.test(page.url()), 'Workspace detail search should preserve the compact lessonslearned continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail lessonslearned compact search');
  await expectBodyText(page, 'match(es) for "lessonslearned"', '/account/work/workspaces detail lessonslearned compact search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail lessonslearned compact search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail lessonslearned compact search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail lessonslearned compact search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail lessonslearned compact search');
  const workspaceLessonsLearnedCompactSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceLessonsLearnedCompactSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the compact lessonslearned continuity query.'
  );
  ...
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=lesson(?:%20|\+)learned/.test(page.url()), 'Workspace detail search should preserve the split lesson learned continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail lesson learned split search');
  await expectBodyText(page, 'match(es) for "lesson learned"', '/account/work/workspaces detail lesson learned split search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail lesson learned split search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail lesson learned split search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail lesson learned split search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail lesson learned split search');
  const workspaceLessonLearnedSplitSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceLessonLearnedSplitSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the split lesson learned continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail lesson learned split search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail lesson learned split search');

  await page.fill('#prepQuery', 'lessons learned');
  await Promise.all([
    page.waitForNavigation({ waitUntil: 'domcontentloaded' }),
    page.getByRole('button', { name: 'Search prep library' }).click()
  ]);
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=lessons(?:%20|\+)learned/.test(page.url()), 'Workspace detail search should preserve the split lessons learned continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail lessons learned split search');
  await expectBodyText(page, 'match(es) for "lessons learned"', '/account/work/workspaces detail lessons learned split search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail lessons learned split search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail lessons learned split search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail lessons learned split search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail lessons learned split search');
  const workspaceLessonsLearnedSplitSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceLessonsLearnedSplitSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the split lessons learned continuity query.'
  );
  ...
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=lesson-learned/.test(page.url()), 'Workspace detail search should preserve the hyphen lesson-learned continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail lesson-learned hyphen search');
  await expectBodyText(page, 'match(es) for "lesson-learned"', '/account/work/workspaces detail lesson-learned hyphen search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail lesson-learned hyphen search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail lesson-learned hyphen search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail lesson-learned hyphen search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail lesson-learned hyphen search');
  const workspaceLessonLearnedHyphenSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceLessonLearnedHyphenSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the hyphen lesson-learned continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail lesson-learned hyphen search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail lesson-learned hyphen search');
  ...
  assert(/\/account\/work\/workspaces\/.+\?prepQuery=lessons-learned/.test(page.url()), 'Workspace detail search should preserve the hyphen lessons-learned continuity prep query in the route.');
  await expectBodyText(page, 'Search results:', '/account/work/workspaces detail lessons-learned hyphen search');
  await expectBodyText(page, 'match(es) for "lessons-learned"', '/account/work/workspaces detail lessons-learned hyphen search');
  await expectBodyText(page, 'Recent governed prep launches', '/account/work/workspaces detail lessons-learned hyphen search');
  await expectBodyText(page, 'Recent travel prefetch receipts', '/account/work/workspaces detail lessons-learned hyphen search');
  await expectBodyText(page, 'Recent aftermath recap packages', '/account/work/workspaces detail lessons-learned hyphen search');
  await expectBodyText(page, 'Next-session carry-forward', '/account/work/workspaces detail lessons-learned hyphen search');
  const workspaceLessonsLearnedHyphenSearchText = await page.locator('body').innerText();
  assert.equal(
    workspaceLessonsLearnedHyphenSearchText.includes('No governed prep packet matched that search yet.'),
    false,
    'Workspace detail search should return at least one governed prep packet for the hyphen lessons-learned continuity query.'
  );
  await assertNoBannedCopy(page, '/account/work/workspaces detail lessons-learned hyphen search');
  await assertNoPageErrors(page, pageErrors, '/account/work/workspaces detail lessons-learned hyphen search');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionreturnpacket', 'compact nextsessionreturnpacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionreturnpackets', 'compact nextsessionreturnpackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionreturnpkt', 'compact nextsessionreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionreturnpkts', 'compact nextsessionreturnpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionreturnbrief', 'compact nextsessionreturnbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionreturnbriefs', 'compact nextsessionreturnbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionreturnbrf', 'compact nextsessionreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionreturnbrfs', 'compact nextsessionreturnbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionsreturnpacket', 'compact nextsessionsreturnpacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionsreturnpackets', 'compact nextsessionsreturnpackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionsreturnpkt', 'compact nextsessionsreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionsreturnpkts', 'compact nextsessionsreturnpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionsreturnbrief', 'compact nextsessionsreturnbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionsreturnbriefs', 'compact nextsessionsreturnbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionsreturnbrf', 'compact nextsessionsreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'nextsessionsreturnbrfs', 'compact nextsessionsreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'sessionreturnpacket', 'compact sessionreturnpacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'sessionreturnpackets', 'compact sessionreturnpackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'sessionreturnpkt', 'compact sessionreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'sessionreturnpkts', 'compact sessionreturnpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'sessionreturnbrief', 'compact sessionreturnbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'sessionreturnbriefs', 'compact sessionreturnbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'sessionreturnbrf', 'compact sessionreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'sessionreturnbrfs', 'compact sessionreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'preplibrarypkt', 'compact preplibrarypkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'preplibrarypkts', 'compact preplibrarypkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'preplibrarybrf', 'compact preplibrarybrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'preplibrarybrfs', 'compact preplibrarybrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'oppositionpkt', 'compact oppositionpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'oppositionpkts', 'compact oppositionpkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostermovepkts', 'compact rostermovepkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostermovementpkt', 'compact rostermovementpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostermovementpkts', 'compact rostermovementpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostermovebrief', 'compact rostermovebrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostermovebriefs', 'compact rostermovebriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostermovementbrief', 'compact rostermovementbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostermovementbriefs', 'compact rostermovementbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostermovebrf', 'compact rostermovebrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostermovebrfs', 'compact rostermovebrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostermovementbrf', 'compact rostermovementbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostermovementbrfs', 'compact rostermovementbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovepacket', 'compact rostersmovepacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovepackets', 'compact rostersmovepackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovepkt', 'compact rostersmovepkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovepkts', 'compact rostersmovepkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovementpacket', 'compact rostersmovementpacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovementpackets', 'compact rostersmovementpackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovementpkt', 'compact rostersmovementpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovementpkts', 'compact rostersmovementpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovebrief', 'compact rostersmovebrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovebriefs', 'compact rostersmovebriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovementbrief', 'compact rostersmovementbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovementbriefs', 'compact rostersmovementbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovebrf', 'compact rostersmovebrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovebrfs', 'compact rostersmovebrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovementbrf', 'compact rostersmovementbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'rostersmovementbrfs', 'compact rostersmovementbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventcontrolpkt', 'compact eventcontrolpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventcontrolpkts', 'compact eventcontrolpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventcontrolbrief', 'compact eventcontrolbrief continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoppkts', 'compact eventoppkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopbrief', 'compact eventopbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopbriefs', 'compact eventopbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopbrf', 'compact eventopbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopbrfs', 'compact eventopbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopspacket', 'compact eventopspacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopspackets', 'compact eventopspackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopspkt', 'compact eventopspkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopspkts', 'compact eventopspkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopsbrief', 'compact eventopsbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopsbriefs', 'compact eventopsbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopsbrf', 'compact eventopsbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventopsbrfs', 'compact eventopsbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationpacket', 'compact eventoperationpacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationpackets', 'compact eventoperationpackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationpkt', 'compact eventoperationpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationpkts', 'compact eventoperationpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationbrief', 'compact eventoperationbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationbriefs', 'compact eventoperationbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationbrf', 'compact eventoperationbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationbrfs', 'compact eventoperationbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationspacket', 'compact eventoperationspacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationspackets', 'compact eventoperationspackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationspkt', 'compact eventoperationspkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationspkts', 'compact eventoperationspkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationsbrief', 'compact eventoperationsbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationsbriefs', 'compact eventoperationsbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationsbrf', 'compact eventoperationsbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'eventoperationsbrfs', 'compact eventoperationsbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignreturnpkt', 'compact campaignreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignreturnpkts', 'compact campaignreturnpkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignreturnbrf', 'compact campaignreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignreturnbrfs', 'compact campaignreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignsreturnpkt', 'compact campaignsreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignsreturnpkts', 'compact campaignsreturnpkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignsreturnbrf', 'compact campaignsreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignsreturnbrfs', 'compact campaignsreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'aftermathreturnpkt', 'compact aftermathreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'aftermathreturnpkts', 'compact aftermathreturnpkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'aftermathreturnbrf', 'compact aftermathreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'aftermathreturnbrfs', 'compact aftermathreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'aftermathsreturnpkt', 'compact aftermathsreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'aftermathsreturnpkts', 'compact aftermathsreturnpkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'aftermathsreturnbrf', 'compact aftermathsreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'aftermathsreturnbrfs', 'compact aftermathsreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'downtimereturnpkt', 'compact downtimereturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'downtimereturnpkts', 'compact downtimereturnpkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'downtimereturnbrf', 'compact downtimereturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'downtimereturnbrfs', 'compact downtimereturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'downtimesreturnpkt', 'compact downtimesreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'downtimesreturnpkts', 'compact downtimesreturnpkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'downtimesreturnbrf', 'compact downtimesreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'downtimesreturnbrfs', 'compact downtimesreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diariesreturnpkt', 'compact diariesreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diariesreturnpkts', 'compact diariesreturnpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diariesreturnbrief', 'compact diariesreturnbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diariesreturnbriefs', 'compact diariesreturnbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diariesreturnbrf', 'compact diariesreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diariesreturnbrfs', 'compact diariesreturnbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diaryreturnpacket', 'compact diaryreturnpacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diaryreturnpackets', 'compact diaryreturnpackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diaryreturnpkt', 'compact diaryreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diaryreturnpkts', 'compact diaryreturnpkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diaryreturnbrf', 'compact diaryreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'diaryreturnbrfs', 'compact diaryreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactsreturnpkt', 'compact contactsreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactsreturnpkts', 'compact contactsreturnpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactsreturnbrief', 'compact contactsreturnbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactsreturnbriefs', 'compact contactsreturnbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactsreturnbrf', 'compact contactsreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactsreturnbrfs', 'compact contactsreturnbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactreturnpacket', 'compact contactreturnpacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactreturnpackets', 'compact contactreturnpackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactreturnpkt', 'compact contactreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactreturnpkts', 'compact contactreturnpkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactreturnbrf', 'compact contactreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'contactreturnbrfs', 'compact contactreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatsreturnpkt', 'compact heatsreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatsreturnpkts', 'compact heatsreturnpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatsreturnbrief', 'compact heatsreturnbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatsreturnbriefs', 'compact heatsreturnbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatsreturnbrf', 'compact heatsreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatsreturnbrfs', 'compact heatsreturnbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatreturnpacket', 'compact heatreturnpacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatreturnpackets', 'compact heatreturnpackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatreturnpkt', 'compact heatreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatreturnpkts', 'compact heatreturnpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatreturnbrief', 'compact heatreturnbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatreturnbriefs', 'compact heatreturnbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatreturnbrf', 'compact heatreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'heatreturnbrfs', 'compact heatreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelofflinepkt', 'compact travelofflinepkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelofflinepkts', 'compact travelofflinepkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelofflinebrief', 'compact travelofflinebrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelofflinebriefs', 'compact travelofflinebriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelofflinebrf', 'compact travelofflinebrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelofflinebrfs', 'compact travelofflinebrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelsofflinepacket', 'compact travelsofflinepacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelsofflinepackets', 'compact travelsofflinepackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelsofflinepkt', 'compact travelsofflinepkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelsofflinepkts', 'compact travelsofflinepkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelsofflinebrief', 'compact travelsofflinebrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelsofflinebriefs', 'compact travelsofflinebriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelsofflinebrf', 'compact travelsofflinebrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travelsofflinebrfs', 'compact travelsofflinebrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travel offline packet', 'split travel offline packet continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travel offline packets', 'split travel offline packets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travel-offline-packet', 'hyphen travel-offline-packet continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'travel-offline-packets', 'hyphen travel-offline-packets continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobileofflinepkt', 'compact mobileofflinepkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobileofflinepkts', 'compact mobileofflinepkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobileofflinebrief', 'compact mobileofflinebrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobileofflinebriefs', 'compact mobileofflinebriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobileofflinebrf', 'compact mobileofflinebrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobileofflinebrfs', 'compact mobileofflinebrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobile offline packet', 'split mobile offline packet continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobile offline packets', 'split mobile offline packets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobile-offline-packet', 'hyphen mobile-offline-packet continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobile-offline-packets', 'hyphen mobile-offline-packets continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousetravelpkt', 'compact safehousetravelpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousetravelpkts', 'compact safehousetravelpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousetravelbrief', 'compact safehousetravelbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousetravelbriefs', 'compact safehousetravelbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousetravelbrf', 'compact safehousetravelbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousetravelbrfs', 'compact safehousetravelbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousestravelpacket', 'compact safehousestravelpacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousestravelpackets', 'compact safehousestravelpackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousestravelpkt', 'compact safehousestravelpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousestravelpkts', 'compact safehousestravelpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousestravelbrief', 'compact safehousestravelbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousestravelbriefs', 'compact safehousestravelbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousestravelbrf', 'compact safehousestravelbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousestravelbrfs', 'compact safehousestravelbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouse travel packet', 'split safehouse travel packet continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouse travel packets', 'split safehouse travel packets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouse-travel-packet', 'hyphen safehouse-travel-packet continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouse-travel-packets', 'hyphen safehouse-travel-packets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouseofflinepacket', 'compact safehouseofflinepacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouseofflinepackets', 'compact safehouseofflinepackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouseofflinepkt', 'compact safehouseofflinepkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouseofflinepkts', 'compact safehouseofflinepkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouseofflinebrief', 'compact safehouseofflinebrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouseofflinebriefs', 'compact safehouseofflinebriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouseofflinebrf', 'compact safehouseofflinebrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouseofflinebrfs', 'compact safehouseofflinebrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousesofflinepacket', 'compact safehousesofflinepacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousesofflinepackets', 'compact safehousesofflinepackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousesofflinepkt', 'compact safehousesofflinepkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousesofflinepkts', 'compact safehousesofflinepkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousesofflinebrief', 'compact safehousesofflinebrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousesofflinebriefs', 'compact safehousesofflinebriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousesofflinebrf', 'compact safehousesofflinebrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehousesofflinebrfs', 'compact safehousesofflinebrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouse offline packet', 'split safehouse offline packet continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouse offline packets', 'split safehouse offline packets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouse-offline-packet', 'hyphen safehouse-offline-packet continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'safehouse-offline-packets', 'hyphen safehouse-offline-packets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehouseofflinepacket', 'compact mobilesafehouseofflinepacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehouseofflinepackets', 'compact mobilesafehouseofflinepackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehouseofflinepkt', 'compact mobilesafehouseofflinepkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehouseofflinepkts', 'compact mobilesafehouseofflinepkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehouseofflinebrief', 'compact mobilesafehouseofflinebrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehouseofflinebriefs', 'compact mobilesafehouseofflinebriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehouseofflinebrf', 'compact mobilesafehouseofflinebrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehouseofflinebrfs', 'compact mobilesafehouseofflinebrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehousesofflinepacket', 'compact mobilesafehousesofflinepacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehousesofflinepackets', 'compact mobilesafehousesofflinepackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehousesofflinepkt', 'compact mobilesafehousesofflinepkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehousesofflinepkts', 'compact mobilesafehousesofflinepkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehousesofflinebrief', 'compact mobilesafehousesofflinebrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehousesofflinebriefs', 'compact mobilesafehousesofflinebriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehousesofflinebrf', 'compact mobilesafehousesofflinebrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilesafehousesofflinebrfs', 'compact mobilesafehousesofflinebrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobile safehouse offline packet', 'split mobile safehouse offline packet continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobile safehouse offline packets', 'split mobile safehouse offline packets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobile-safehouse-offline-packet', 'hyphen mobile-safehouse-offline-packet continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobile-safehouse-offline-packets', 'hyphen mobile-safehouse-offline-packets continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmopspkt', 'compact gmopspkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmopspkts', 'compact gmopspkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationpkt', 'compact gmoperationpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationpkts', 'compact gmoperationpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationspacket', 'compact gmoperationspacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationspackets', 'compact gmoperationspackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationspkt', 'compact gmoperationspkt continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmopsbrfs', 'compact gmopsbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationbrief', 'compact gmoperationbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationbriefs', 'compact gmoperationbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationbrf', 'compact gmoperationbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationbrfs', 'compact gmoperationbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationsbrief', 'compact gmoperationsbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationsbriefs', 'compact gmoperationsbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationsbrf', 'compact gmoperationsbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmoperationsbrfs', 'compact gmoperationsbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gmcontrolbrief', 'compact gmcontrolbrief continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteropspkts', 'compact gamemasteropspkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationpacket', 'compact gamemasteroperationpacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationpackets', 'compact gamemasteroperationpackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationpkt', 'compact gamemasteroperationpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationpkts', 'compact gamemasteroperationpkts continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationspacket', 'compact gamemasteroperationspacket continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationspackets', 'compact gamemasteroperationspackets continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationspkt', 'compact gamemasteroperationspkt continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteropsbrfs', 'compact gamemasteropsbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationbrief', 'compact gamemasteroperationbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationbriefs', 'compact gamemasteroperationbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationbrf', 'compact gamemasteroperationbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationbrfs', 'compact gamemasteroperationbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationsbrief', 'compact gamemasteroperationsbrief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationsbriefs', 'compact gamemasteroperationsbriefs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationsbrf', 'compact gamemasteroperationsbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemasteroperationsbrfs', 'compact gamemasteroperationsbrfs continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'gamemastercontrolbrief', 'compact gamemastercontrolbrief continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignworkspacev4', 'compact campaignworkspacev4 continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'workspacev4brief', 'compact workspacev4brief continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'workspacev4briefs', 'compact workspacev4briefs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilecompanionreturnpkt', 'compact mobilecompanionreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilecompanionsreturnpkts', 'compact mobilecompanionsreturnpkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilecompanionreturnbrf', 'compact mobilecompanionreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'mobilecompanionsreturnbrfs', 'compact mobilecompanionsreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignmobilecompanionreturnpkt', 'compact campaignmobilecompanionreturnpkt continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignmobilecompanionsreturnpkts', 'compact campaignmobilecompanionsreturnpkts continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignmobilecompanionreturnbrf', 'compact campaignmobilecompanionreturnbrf continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'campaignmobilecompanionsreturnbrfs', 'compact campaignmobilecompanionsreturnbrfs continuity prep');
  ...
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'stalecache', 'compact stalecache continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'stalecaches', 'compact stalecaches continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'staleofflinecache', 'compact staleofflinecache continuity prep');
  await assertWorkspacePrepQuerySearch(page, pageErrors, 'staleofflinecaches', 'compact staleofflinecaches continuity prep');
