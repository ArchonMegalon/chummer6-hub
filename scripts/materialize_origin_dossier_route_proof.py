...
def sha256_response(response: requests.Response) -> str:
...
def build_publication_index(
    ...
    entry["ebookAudiobookshelfImportReceiptPath"] = entry.get("ebookAudiobookshelfImportReceiptPath") or str(branch / "dossier/audiobookshelf-dossier-import.receipt.json")
    entry["coverConsistencyReceiptPath"] = entry.get("coverConsistencyReceiptPath") or str(branch / "cover-consistency-strict.receipt.json")
    entry["audiobookPath"] = entry.get("audiobookPath") or str(branch / "audiobook" / f"{context.runner_name.lower()}-origin.m4b")
    entry["audiobookshelfImportReceiptPath"] = entry.get("audiobookshelfImportReceiptPath") or str(branch / "audiobook/audiobookshelf-import.receipt.json")
    entry["finalNoFallbackNoSentinelAuditReceiptPath"] = entry.get("finalNoFallbackNoSentinelAuditReceiptPath") or str(branch / "final-no-fallback-no-sentinel-audit.receipt.json")
    ...
    with tempfile.TemporaryDirectory(prefix="chummer-origin-route-") as temp_dir:
        ...
        with identity:
            ...
            with app:
                ...
                selected_face_cover_route_visible = expected_cover_route in detail_text
                selected_face_cover_visible = (
                    selected_face_cover_marker_visible
                    and selected_face_cover_alt_visible
                    and selected_face_cover_route_visible
                )
                read_section_visible = 'id="origin-edition-read"' in detail_text and 'data-origin-edition-tab="read"' in detail_text
                listen_section_visible = 'id="origin-edition-listen"' in detail_text and 'data-origin-edition-tab="listen"' in detail_text
                watch_section_visible = 'id="origin-edition-watch"' in detail_text and 'data-origin-edition-tab="watch"' in detail_text
                for expected in (
                ...
                require(selected_face_cover_marker_visible, "selected face cover marker missing", failures)
                require(selected_face_cover_alt_visible, "selected face cover alt missing", failures)
                require(selected_face_cover_route_visible, "selected face cover route missing", failures)
                require(read_section_visible, "read tab section missing", failures)
                require(listen_section_visible, "listen tab section missing", failures)
                require(watch_section_visible, "watch tab section missing", failures)
                ...
                require(bool(expected_cover_sha), "expected cover sha missing from live import evidence", failures)
                require(bool(expected_book_sha), "expected book sha missing from live import evidence", failures)
                require(bool(expected_video_sha), "expected video sha missing from live import evidence", failures)
                require(cover_sha == expected_cover_sha, "cover route sha mismatch", failures)
                require(book_sha == expected_book_sha, "book route sha mismatch", failures)
                require(video_sha == expected_video_sha, "video route sha mismatch", failures)
                ...
                payload: dict[str, Any] = {
                    ...
                    "listenTabVisible": "href=\"#origin-edition-listen\"" in detail_text and listen_section_visible,
                    "watchTabVisible": "href=\"#origin-edition-watch\"" in detail_text and watch_section_visible,
                    ...
                    "selectedFaceCoverVisible": selected_face_cover_visible,
                    "selected_face_cover_marker_visible": selected_face_cover_marker_visible,
                    "selected_face_cover_alt_visible": selected_face_cover_alt_visible,
                    "selected_face_cover_route_visible": selected_face_cover_route_visible,
                    "read_tab_visible": "href=\"#origin-edition-read\"" in detail_text and read_section_visible,
                    "read_section_visible": read_section_visible,
                    "listen_tab_visible": "href=\"#origin-edition-listen\"" in detail_text and listen_section_visible,
                    "listen_section_visible": listen_section_visible,
                    "watch_tab_visible": "href=\"#origin-edition-watch\"" in detail_text and watch_section_visible,
                    "watch_section_visible": watch_section_visible,
                    ...
                    "selected_face_cover_visible": selected_face_cover_visible,
