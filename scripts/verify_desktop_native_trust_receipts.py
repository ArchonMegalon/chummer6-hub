...
REQUIRED_SOURCE_MARKERS = {
    Path("Chummer.Run.Api/Controllers/InstallLinkingController.cs"): [
        ...
        "FallbackPosture: releaseAvailabilityAllowed",
        "BuildNativeNextSafeAction(updateAvailable, leadSupportCase, continuation)",
        "NativePrimaryActionHref: releaseAvailabilityAllowed ? BuildNativeContinuationPrimaryActionHref(updateAvailable, leadSupportCase) : NativeContinuationHref",
        ...
        "RecoveryAction: BuildNativeRecoveryAction(installation, continuation, releaseAvailabilityAllowed)",
        "private static string BuildNativeRecoveryAction(",
        "bool releaseAvailabilityAllowed) => releaseAvailabilityAllowed ? continuation?.NextSafeAction ?? $\"Return {NormalizeResponseValue(installation.HeadId)} {NormalizeResponseValue(installation.Platform)} {NormalizeResponseValue(installation.Arch)} to this linked copy for recovery; claim-code and account browser links remain fallback context only.\" : \"Keep the installed copy unchanged and use grant-bound support for recovery; no installer or updater handoff is authority-approved.\"",
        "claim-code and account browser links remain fallback context only.",
        "BuildNativeSupportNextSafeAction(item, updateAvailable, releaseAvailabilityAllowed)",
        "BuildNativeSupportNextSafeAction(presented, updateAvailable, releaseAvailabilityAllowed)",
        "private static string BuildNativeSupportNextSafeAction( SupportCasePresentationViewModel item, bool updateAvailable, bool releaseAvailabilityAllowed)",
        ...
        "BuildNativeSupportCaseActionHref(item, updateAvailable, releaseAvailabilityAllowed)",
        ...
        "ContainsSingleEncodedPathSeparator(href)",
        "ContainsSingleEncodedPathSeparator(decoded)",
        ...
        "IsTrustedNativeInstallRailPublicHost(absoluteUri.Host)",
        ...
        "BuildNativeInstallSupportRequest( request, installation, receipt, manifest, releaseArtifact, continuation, updateAvailable, releaseAvailabilityAllowed)",
        ...
        "AppendNativeReleaseRecoveryTruthDetail( detail, manifest, releaseArtifact, continuation, updateAvailable, releaseAvailabilityAllowed)",
        ...
        "FindNativeRequestedActionFragmentIndex(sanitized)",
        "private static int FindNativeRequestedActionFragmentIndex(string href)",
        "IsHtmlNumericEntityHash(href, index)",
        "private static bool IsHtmlNumericEntityHash(string href, int hashIndex)",
        "Uri.IsHexDigit(href[hexIndex])",
        "char.IsDigit(href[digitIndex])",
        ...
        'component.Contains("&#x3d;", StringComparison.OrdinalIgnoreCase)',
        "SanitizeInstallLinkSecretQueryPart(part, out bool partRedacted)",
        "FindNextInstallLinkSecretSeparator(body, partStart)",
        "private static (int Index, int Length) FindNextInstallLinkSecretSeparator(string body, int startIndex)",
        "MatchHtmlEntityInstallLinkSecretSeparator(body, index)",
        ...
        "FindInstallLinkSecretEquals(part)",
        "DecodeInstallLinkSecretKey(rawKey)",
        "private static (int Index, int Length) FindInstallLinkSecretEquals(string part)",
        "MatchHtmlEntityInstallLinkSecretEquals(part, index)",
        "MatchHtmlEntityInstallLinkSecretEquals(body, index)",
        ...
        'remaining.StartsWith("&equals;", StringComparison.OrdinalIgnoreCase)',
        'remaining.StartsWith("&#61;", StringComparison.OrdinalIgnoreCase)',
        'remaining.StartsWith("&#x3d;", StringComparison.OrdinalIgnoreCase)',
        ...
        "NativeRollbackHref: \"/api/v1/install-linking/continuation/rollback\"",
        "NativeRecoveryHref: NativeRecoveryHref",
        "PrimaryActionHref: BuildNativeSupportCaseActionHref(presented, updateAvailable, releaseAvailabilityAllowed)",
        "CurrentReleaseVersion: manifest.Version",
        "FallbackPosture: releaseAvailabilityAllowed",
        ...
        "NativePrimaryActionHref: releaseAvailabilityAllowed ? NativeUpdateHref : NativeContinuationHref",
        ...
        "NativePrimaryActionHref: releaseAvailabilityAllowed ? NativeRollbackHref : NativeContinuationHref",
        "NativeUpdateHref: \"/api/v1/install-linking/continuation/update\"",
        "NativeContinuationHref: \"/api/v1/install-linking/continuation\"",
        "NativeSupportHref: \"/api/v1/install-linking/continuation/support\"",
        "NativeRecoveryHref: NativeRecoveryHref",
        ...
        "PreserveInstallLinkCallbackComponent(fragment, \"#\")",
    ...
    Path("Chummer.Run.Api/Services/Support/SupportCasePresentationService.cs"): [
        ...
        "Installs only when you need to relink or reclaim that copy.",
    ],
    ...
    Path("tests/RunServicesVerification/InstallLinkingContinuationVerification.cs"): [
        ...
        "Native support action sanitizer should preserve native update actions when encoded slash state appears only outside the route path.",
        ...
        "Native support action sanitizer should fail closed instead of preserving double-encoded native-looking actions.",
        "Native support action sanitizer should fail closed instead of preserving double-encoded Windows-style native-looking actions.",
        ...
        "Native support action sanitizer should only use native update when install-readiness requires an update.",
        ...
        "Native support case should redact semicolon-separated query access-token keys from the requested action.",
        "Native support case should redact semicolon-separated query grant-id keys from the requested action.",
        "Native support case should redact semicolon-separated fragment claim-code keys from the requested action.",
        "Native support case should redact semicolon-separated fragment installed-build receipt keys from the requested action.",
        "Native support case should not persist semicolon-separated access-token values from requested-action hints.",
        "Native support case should redact double-encoded query access-token keys from the requested action.",
        "Native support case should redact double-encoded query grant-id keys from the requested action.",
        "Native support case should redact double-encoded fragment claim-code keys from the requested action.",
        "Native support case should redact double-encoded fragment installed-build receipt keys from the requested action.",
        "Native support case should not persist double-encoded access-token values from requested-action hints.",
        "Native support case should redact query secrets after encoded separators.",
        "Native support case should redact query secrets after encoded hash separators.",
        "Native support case should redact fragment secrets after double-encoded separators.",
        "Native support case should redact fragment secrets after double-encoded hash separators.",
        "Native support case should redact encoded query access-token keys after HTML ampersand separators.",
        ...
        "Native support case should not persist receipt values after double-encoded hash separators.",
        "Native support case should not persist access-token values after HTML entity separators.",
        ...
        "Native support case should redact fragment installed-build receipt values after HTML named equals separators.",
        "Native support case should not persist access-token values after HTML entity equals separators.",
        "Native support case should not persist grant-id values after HTML entity equals separators.",
        "Native support case should not persist claim-code values after HTML entity equals separators.",
        "Native support case should not persist installed-build receipt values after HTML entity equals separators.",
        "Native support case should redact short-lived bootstrap ticket query values from requested-action hints.",
        "Native support case should not persist short-lived bootstrap ticket values from requested-action hints.",
        ...
        "App-local callback redirects should preserve desktop listener fragment state.",
        "App-local callback redirects should strip stale fragment grant tokens.",
        "App-local callback redirects should keep grant callback transport authoritative when fragment state is present.",
        ...
        "Native support follow-through should not tell the desktop app to open Installs.",
        ...
        "New native support cases should not tell the desktop app to open Installs.",
        ...
        "Channel-only install help should not route reporter-ready support follow-through into Installs without installed build and device truth.",
...
def _verify_required_source_markers(errors: list[str], repo_root: Path) -> None:
    for relative_path, markers in REQUIRED_SOURCE_MARKERS.items():
        ...
        text = path.read_text(encoding="utf-8")
        for marker in markers:
            if not _block_contains_marker(text, marker):
