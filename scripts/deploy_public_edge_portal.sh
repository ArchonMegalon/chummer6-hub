...
CORE_RUNTIME_BUNDLE_SOURCE_INPUT="$("$TRUSTED_DIRNAME" -- "$SOURCE_ROOT")/core-runtime-package-plane-c06f22c185c7b733637fdb76b3cf333f31716781-input"
if ! CORE_RUNTIME_BUNDLE_SOURCE="$(
  "$TRUSTED_REALPATH" -e -- "$CORE_RUNTIME_BUNDLE_SOURCE_INPUT"
)" \
  || [[ "$CORE_RUNTIME_BUNDLE_SOURCE" != "$CORE_RUNTIME_BUNDLE_SOURCE_INPUT" ]]; then
  echo "sealed Core runtime bundle input is missing or path-aliased" >&2
  exit 2
fi
HUB_PACKAGE_FEED_SOURCE_INPUT="$("$TRUSTED_DIRNAME" -- "$SOURCE_ROOT")/hub-package-feed-sh1852ea4eef6d-input"
if ! HUB_PACKAGE_FEED_SOURCE="$(
  "$TRUSTED_REALPATH" -e -- "$HUB_PACKAGE_FEED_SOURCE_INPUT"
)" \
  || [[ "$HUB_PACKAGE_FEED_SOURCE" != "$HUB_PACKAGE_FEED_SOURCE_INPUT" ]]; then
  echo "sealed Hub package feed input is missing or path-aliased" >&2
  exit 2
fi
...
configure_compose_operation() {
  ...
  compose_command=(
    ...
    CHUMMER_CORE_RUNTIME_BUNDLE_SOURCE="$CORE_RUNTIME_BUNDLE_SOURCE"
    CHUMMER_HUB_PACKAGE_FEED_SOURCE="$HUB_PACKAGE_FEED_SOURCE"
