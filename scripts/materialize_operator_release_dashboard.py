...
SHARED_WORKSPACE_ROOT = Path(
    os.environ.get("CHUMMER_SHARED_WORKSPACE_ROOT") or "/docker/chummercomplete"
)
SHARED_REGISTRY_ROOT = (
    SHARED_WORKSPACE_ROOT / "chummer-hub-registry" / ".codex-studio" / "published"
)
SHARED_RUN_SERVICES_ROOT = Path(
    os.environ.get("CHUMMER_SHARED_RUN_SERVICES_ROOT")
    or "/docker/chummercomplete/chummer.run-services"
)
...
TELEGRAM_TEXT_DELIVERY_ROOT = (
    SHARED_WORKSPACE_ROOT / "_completion" / "telegram_text_delivery"
)
PUBLIC_RELEASE_SNAPSHOT_PATH = (
    SHARED_WORKSPACE_ROOT
    / ".codex-studio"
    / "published"
    / "PUBLIC_RELEASE_SNAPSHOT.generated.json"
)
PUBLIC_RELEASE_SNAPSHOT_READONLY_AUDIT_PATH = (
    SHARED_WORKSPACE_ROOT
    / ".codex-studio"
    / "published"
    / "PUBLIC_RELEASE_SNAPSHOT_READONLY_AUDIT.generated.json"
)
ROOT_RELEASE_BLOCKERS_PATH = SHARED_WORKSPACE_ROOT / "RELEASE_BLOCKERS.generated.json"
...
WORKSPACE_PORTAL_RELEASE_CHANNEL_CANDIDATES = (
    ...
    PUBLISHED_ROOT / "portal" / "RELEASE_CHANNEL.generated.json",
...
def resolve_completion_root() -> Path:
    explicit = os.environ.get("CHUMMER_OPERATOR_COMPLETION_ROOT") or os.environ.get(
        "CHUMMER_UI_COMPLETION_ROOT"
    )
    if explicit:
        return Path(explicit).expanduser()
    candidates = [
        ROOT / "_completion" / "chummer_run_redesign_closure",
        SHARED_WORKSPACE_ROOT / "_completion" / "chummer_run_redesign_closure",
    ]
...
def resolve_release_channel_path() -> Path:
    explicit = os.environ.get("CHUMMER_OPERATOR_RELEASE_CHANNEL_PATH") or os.environ.get(
        "CHUMMER_RELEASE_CHANNEL_PATH"
    )
    candidates = unique_paths(
        [
            (
                ...
                if explicit
                else REGISTRY_ROOT / "RELEASE_CHANNEL.generated.json"
            ),
...
def release_channel_semantic_failures(payload: dict[str, Any]) -> list[str]:
    ...
    return normalized_string_list(failures)
...
def windows_operator_request_artifacts(
    ...
    *,
    ...
    if refresh_windows_runtime_receipts and runtime_refresh_authorized:
    ...
    *,
    ...
    if windows_visual_audit_intake_request_receipt_path.is_file() and isinstance(windows_visual_audit_intake_request, dict):
        windows_runtime_refresh_authorized = (
            str(windows_visual_audit_intake_request_verifier.get("status") or "")
            .strip()
            .lower()
            == "pass"
            and windows_visual_audit_intake_request_verifier.get("recovery_pack_pass")
            is True
            and windows_visual_audit_intake_request_verifier.get(
                "runtime_refresh_commands_trusted"
            )
            is True
            and not normalized_string_list(
        ...
        checks["windows_installer_visual_audit"]["operator_request_artifacts"] = windows_operator_request_artifacts(
            ...
            refresh_windows_runtime_receipts=(
                refresh_windows_runtime_receipts
            ),
        ...
        checks["release_ready"].update(
            {
                ...
                "current_release_truth_blockers": safe_current_release_truth_blockers(),
            }
        )
...
def parse_args() -> argparse.Namespace:
    ...
    release_ready_group = parser.add_mutually_exclusive_group()
    release_ready_group.add_argument(
    ...
    release_ready_group.add_argument(
        ...
        action="store_false",
        help=(
            "Include the previous release_ready receipt as a release-blocking input. "
            "Use only when auditing that receipt rather than producing current dependency truth."
        ),
    )
