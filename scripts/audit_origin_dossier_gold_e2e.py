...
TRUSTED_AUDIOBOOKSHELF_HOSTS = {"audio.chummer.run", "audiobookshelf.chummer.run"}
...
def _chummer_owner_url(url: str, project_id: str, suffix: str = "") -> bool:
    parsed = urlparse(url)
    expected_path = f"/account/work/origin-dossiers/{project_id}{suffix}"
    return (
        parsed.scheme.lower() == "https"
...
def _audit_live_import(path: Path) -> tuple[dict[str, Any], list[str]]:
    ...
    if project_id:
        _add_issue(issues, _chummer_owner_url(_string(payload.get("chummerRunOwnerUrl")), project_id), "owner_url_not_chummer_run")
        _add_issue(issues, _chummer_owner_url(_string(request.get("bookArtifactUrl")), project_id, "/book"), "book_url_not_chummer_run")
        _add_issue(issues, _chummer_owner_url(_string(request.get("storySceneCoverUrl")), project_id, "/cover"), "cover_url_not_chummer_run")
        _add_issue(issues, _chummer_owner_url(_string(request.get("dossierVideoUrl")), project_id, "/video"), "video_url_not_chummer_run")
    for key in (
        "sourcePacketSha256",
    ...
    for key in (
        ...
        _add_issue(issues, bool(value) and Path(value).is_file(), f"import_artifact_missing:{key}")
...
def _audit_ea_delivery(path: Path, share_url: str) -> tuple[dict[str, Any], list[str]]:
    issues: list[str] = []
    payload = _read_json(path)
...
def _audit_browser_proof(path: Path, project_id: str, share_url: str) -> tuple[dict[str, Any], list[str]]:
    issues: list[str] = []
    payload = _read_json(path)
    ...
    _add_issue(issues, _string(payload.get("base_url")) == "https://chummer.run", "browser_base_url_not_deployed_chummer_run")
    _add_issue(issues, _chummer_owner_url(_string(payload.get("owner_detail_page")), project_id), "browser_owner_detail_not_chummer_run")
    _add_issue(issues, _chummer_owner_url(_string(payload.get("selected_face_cover_url")), project_id, "/cover"), "browser_cover_url_not_chummer_run")
    _add_issue(issues, _chummer_owner_url(_string(payload.get("listen_url")), project_id, "/listen"), "browser_listen_url_not_chummer_run")
...
def audit(
    *,
    ...
    if ea_delivery_receipt.is_file() and live_evidence["audiobookshelf_share_url"]:
        delivery_evidence, delivery_issues = _audit_ea_delivery(ea_delivery_receipt, live_evidence["audiobookshelf_share_url"])
