...
def runtime_refresh_command_issues(
    path: Path,
    payload: dict[str, Any],
    *,
    module: Any | None = None,
) -> list[str]:
    """Require generated, repository-owned argv before any receipt command executes."""

    request_module = module or load_module()
    artifact_intake = (
        payload.get("artifact_intake")
        if isinstance(payload.get("artifact_intake"), dict)
        else {}
    )
    expected_commands = {
        "auto_import_command": request_module.build_auto_import_command(path),
        "auto_import_watch_command": request_module.build_auto_import_command(
            path,
            wait_seconds=900,
            poll_seconds=10,
            refresh_intake_request=True,
        ),
        "watcher_start_command": request_module.build_watcher_manager_command(
            "start",
            path,
        ),
        "watcher_status_command": request_module.build_watcher_manager_command(
            "status",
            path,
        ),
        "watcher_stop_command": request_module.build_watcher_manager_command(
            "stop",
            path,
        ),
    }
    ...
    for field, expected in expected_commands.items():
        if str(artifact_intake.get(field) or "").strip() != expected:
            issues.append(f"artifact_intake_{field}_binding_mismatch")

    expected_paths = {
        "watcher_state_path": request_module.DEFAULT_WATCHER_STATE_PATH,
        "watcher_pid_file": request_module.DEFAULT_WATCHER_PID_FILE,
        "watcher_log_path": request_module.DEFAULT_WATCHER_LOG_FILE,
    }
    for field, expected in expected_paths.items():
        if not paths_match(artifact_intake.get(field), expected):
            issues.append(f"artifact_intake_{field}_binding_mismatch")
    ...
    if request_status == "external_artifact_required" and preferred_drop_path:
        expected_import_command = request_module.build_import_command(
            Path(preferred_drop_path),
            path,
        )
        if import_command != expected_import_command:
            issues.append("artifact_intake_import_command_binding_mismatch")
...
def verify(path: Path, *, require_pass: bool = False) -> tuple[bool, dict[str, Any]]:
    ...
    runtime_refresh_issues = runtime_refresh_command_issues(
        path,
        payload,
        module=module,
    )
    structural_issues: list[str] = list(runtime_refresh_issues)
    ...
    result = {
        ...
        "runtime_refresh_commands_trusted": not runtime_refresh_issues,
